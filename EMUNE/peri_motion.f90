...
subroutine peri_motion_velocity_verlet
	...
	do mi=1,nnodes
		...
		if(nodes(mi)%iflag==.true.) goto 100
		...
		do js=1,mfam
			...
			if(nodes(mj)%iflag==.true.) continue  ! hanging node, no calculation
...
!			if(broke(mi,mj).eq.1) then
			if(broke(js,mi).eq.1) then
				nbroke=nbroke+1
			endif
		end do
...
!			nodes(mi)%new_veloc(2) = (ext2(mi)+f2+fac*nodes(mi)%veloc(2))/(fac+dampy)
			if( dynamic_or_static == 1) then ! dynamic
				if (iter.eq.1) then ! Forward-Euler scheme for the first step
					fac = dt/denst
...
!    write (*,'(f25.12)') dc3
    ...
    if ((iter.eq.1).or.(modified_mesh.eq..true.)) then
  	    do mi=1,nnodes
   		    ! Get the family of node mi (all the nodes within its horizon, delta).
	        delta=nodes(mi)%horizon_size
   		    call get_family(mi)
...
!   1st step & 2nd step (intermediate velocity update and position update)
    do mi=1,nnodes
   	    if(omitt(mi).or.(nodes(mi)%iflag==.true.)) then
		    nodes(mi)%new_veloc(1) = zero
		    nodes(mi)%new_veloc(2) = zero
		    nodes(mi)%new_disp(1) = nodes(mi)%disp(1)
    	    nodes(mi)%new_disp(2) = nodes(mi)%disp(2)
...
subroutine shortrange_motion
      ...
      do mi=1,nnodes
         if(nodes(mi)%iflag==.true.) continue
