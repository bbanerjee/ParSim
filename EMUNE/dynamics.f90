	...
	call cpu_time(time1)
		  ...
		  if (do_adap_refinement == 1)then ! If  is  =1 then the refinement and coarsening will be performed
			 if(iter.eq.1) then
				  ...
				  call sort_ref
				  if (introduce_precrack.eq.1) call initial_broken_bond
!			      call Refine
				  broke = 0
			 	  call compute_critical_strain
				  call sort_ref
...
!			 if((mod(iter,100).eq.0).and.(iter>=40))then			
			 if((mod(iter,10).eq.0).and.(iter>=800))then			
...
!			      call Refine
	...
	print *, 'damage, computational time(seconds):',time2-time1
		  ...
		  ! Output nodal information every snapshots_frequence interation 	
		  if(iter==1)then
!				write(667,'(i6)') nnodes
!				do i=1,nnodes
...
!		  call intergrate_disp ! integrate the new velocity to obtain the new displacement of each node
		  ...
		  ! Output nodal information every snapshots_frequence interation 	
		  if(mod(iter,snapshots_frequence).eq.0)then
!				write(667,'(i6)') nnodes
!				do i=1,nnodes
!						write(666,'(3   f20.6)') nodes(i)%pos(1)+nodes(i)%disp(1),nodes(i)%pos(2)+nodes(i)%disp(2),nodes(i)%damage_index
!				enddo
...
!___02262009_____________________________________________    
	...
	mbd = 0  ! mbd.eq.1: apply boundary condition, mpd.eq.0: no B.C
...
!	enddo
	...
	do i=1,icount1  ! vanished the thickness effect
	  ...
	  if(i.ge.2.and.i.le.icount1-1) then
	    ...
	    if(i.eq.1) ext1(num)=-force_mag*abs(ytempbotton(i+1)-ytempbotton(i))/(nodes(num)%volume*2.0d0)
	    if(i.eq.icount1) ext1(num)=-force_mag*abs(ytempbotton(i)-ytempbotton(i-1))/(nodes(num)%volume*2.0d0)
	  endif
	enddo
...
!	  else
!	    if(i.eq.1) ext1(num)=force_mag*abs(ytemptop(i+1)-ytemptop(i))/(nodes(num)%volume*2.0d0)
!	    if(i.eq.icount2) ext1(num)=force_mag*abs(ytemptop(i)-ytemptop(i-1))/(nodes(num)%volume*2.0d0)
	  endif
	enddo
...
!	    if(i.eq.1) ext1(num)=-force_mag*abs(ytempbotton(i+1)-ytempbotton(i))/(nodes(num)%volume*2.0d0)
!	    if(i.eq.icount1) ext1(num)=-force_mag*abs(ytempbotton(i)-ytempbotton(i-1))/(nodes(num)%volume*2.0d0)
	    if(i.eq.1) ext2(num)=-force_mag*dabs(xtempbotton(i+1)-xtempbotton(i))/(nodes(num)%volume*2.d0)
	    if(i.eq.icount1) ext2(num)=-force_mag*dabs(xtempbotton(i)-xtempbotton(i-1))/(nodes(num)%volume*2.d0)
!	    if(i.eq.1) ext1(num)=-force_mag
!	    if(i.eq.icount1) ext1(num)=-force_mag
	  endif
	enddo
...
!	    if(i.eq.1) ext1(num)=force_mag*abs(ytemptop(i+1)-ytemptop(i))/(nodes(num)%volume*2.0d0)
!	    if(i.eq.icount2) ext1(num)=force_mag*abs(ytemptop(i)-ytemptop(i-1))/(nodes(num)%volume*2.0d0)
!	    if(i.eq.1) ext1(num)=force_mag*abs(ytemptop(i+1)-ytemptop(i))/(nodes(num)%volume*4.0d0)
!	    if(i.eq.icount2) ext1(num)=force_mag*abs(ytemptop(i)-ytemptop(i-1))/(nodes(num)%volume*4.0d0)
	    if(i.eq.1) ext2(num)=force_mag*dabs(xtemptop(i+1)-xtemptop(i))/(nodes(num)%volume*2.0d0)
	    if(i.eq.icount2) ext2(num)=force_mag*dabs(xtemptop(i)-xtemptop(i-1))/(nodes(num)%volume*2.0d0)
!	    if(i.eq.1) ext1(num)=force_mag
!	    if(i.eq.icount2) ext1(num)=force_mag
	  endif
	enddo
...
!	integer(4) :: ncorrespond_botton(nnodes), ncorrespond_top(nnodes)
	...
	do i=1,icount1  ! vanished the thickness effect
	  ...
	  if(i.ge.2.and.i.le.icount1-1) then
	    ...
	    if(i.eq.1) ext1(num)=-force_mag*abs(ytempbotton(i+1)-ytempbotton(i))/2.0d0
	    if(i.eq.icount1) ext1(num)=-force_mag*abs(ytempbotton(i)-ytempbotton(i-1))/2.0d0
	  endif
	enddo
	...
	do i=1,icount2
	  ...
	  if(i.ge.2.and.i.le.icount2-1) then
	    ...
	    if(i.eq.1) ext1(num)=force_mag*abs(ytemptop(i+1)-ytemptop(i))/2.0d0
	    if(i.eq.icount2) ext1(num)=force_mag*abs(ytemptop(i)-ytemptop(i-1))/2.0d0
	  endif
	enddo
...
!	endif
	...
	do i=1,icount1  ! vanished the thickness effect
	  ...
	  if(i.ge.2.and.i.le.icount1-1) then
	    ...
	    if(i.eq.1) ext2(num)=-0.13*force_mag2*dabs(xtempbotton(i+1)-xtempbotton(i))/(nodes(num)%volume*2.d0)
	    if(i.eq.icount1) ext2(num)=-0.13*force_mag2*dabs(xtempbotton(i)-xtempbotton(i-1))/(nodes(num)%volume*2.d0)
	  endif
	enddo
	do i=1,icount2
	  num=ncorrespond_top(i)
	  if(i.ge.2.and.i.le.icount2-1) then
	    ...
	    if(i.eq.1) ext2(num)=-force_mag2*dabs(xtemptop(i+1)-xtemptop(i))/(nodes(num)%volume*2.0d0)
	    if(i.eq.icount2) ext2(num)=-force_mag2*dabs(xtemptop(i)-xtemptop(i-1))/(nodes(num)%volume*2.0d0)
	  endif
!	  print *, ext2(num)
	enddo
...
!********************************************************************
	...
	integer :: i, j, icount1, icount2, num
!	real(8) :: xtempbotton(nnodes),ytempbotton(nnodes), xtemptop(nnodes), ytemptop(nnodes)
