...
\Beq
 \text{Find}~~\Bf_g~~\text{that minimizes}~~ E = \IntOmega w(\Bx)\Norm{\Be(\Bx)}{}^2\, dV~~
   \text{where}~~\IntOmega w(\Bx)\,dV = 1 \,. 
\Eeq
...
\Beq
  ...
  \IntOmega w(\Bx)\left[\Partial{\Be}{\Bf_g}\cdot\Be(\Bx) + \Be(\Bx)\cdot\Partial{\Be}{\Bf_g}\right]\,dV = \Bzero
\Eeq
...
Therefore,
\Beq
  \IntOmega w(\Bx) S_g(\Bx)\Be(\Bx)\,dV = \Bzero
  \quad \implies \quad
  \IntOmega w(\Bx) S_g(\Bx)\left[\sum_{g'} \Bf_{g'} S_{g'}(\Bx) - \sum_p \Bf_p \chi_p(\Bx) \right]\,dV = \Bzero \,.
\Eeq
Rearrangement of the above equation leads to
\Beq \label{eq:g_p_map_0}
  \sum_g \Bf_g \IntOmega w(\Bx) S_{g'}(\Bx) S_g(\Bx)\,dV =
    \sum_p \Bf_p \IntOmega w(\Bx) S_{g'}(\Bx) \chi_p(\Bx)\,dV \,.
\Eeq
Define
\Beq \label{eq:Sgp_def}
  \boxed{
  A_{g'g} := \IntOmega w(\Bx) S_{g'}(\Bx) S_g(\Bx)\,dV
  \quad \Tand \quad
  B_{g'p} := \IntOmega w(\Bx) S_{g'}(\Bx) \chi_p(\Bx)\,dV \,.
  }
\Eeq
...
In that case
\Beq
  A_{g'} = \sum_g A_{g'g} = \IntOmega w(\Bx) S_{g'}(\Bx) \sum_g S_g(\Bx) \,dV
     = \IntOmega w(\Bx) S_{g'}(\Bx) \,dV
\Eeq
...
\Beq \label{eq:particle_to_grid}
  ...
  A_{g} = \IntOmega w(\Bx) S_{g}(\Bx) \,dV~,~~
  B_{gp} = \IntOmega w(\Bx) S_{g}(\Bx) \chi_p(\Bx)\,dV \,.
\Eeq
\end{NoteBox}
...
may, alternatively, define $f_p$ as
\Beq \label{eq:f_p}
  \Bf_p = \frac{1}{W_p} \IntOmegap \Bf(\Bx) \omega_p(\Bx)\,dV ~,~~
  W_p := \IntOmegap \omega_p(\Bx)\,dV
\Eeq
...
\Beq
  \Bf_p = \frac{1}{W_p} \IntOmegap \left[\sum_g \Bf_g S_g(\Bx)\right] \omega_p(\Bx)\,dV 
        = \sum_g \Bf_g \left[\frac{1}{W_p} \IntOmegap S_g(\Bx) \omega_p(\Bx)\,dV\right] \,.
\Eeq
...
\Beq
  \Bf_p \approx \sum_g \Bf_g \left[\frac{1}{W^\star_p} \int_{\Omega_p^\star} S_g(\Bx) \omega^\star_p(\Bx)\,dV\right] ~,~~
  W^\star_p := \int_{\Omega_p^\star}\omega^\star_p(\Bx)\,dV
\Eeq
where the alternative weight function $\omega^\star_p(\Bx)$ is defined as
\Beq
  \omega^\star_p(\Bx) := \omega_p(\Bx) \,\chi^\star_p(\Bx)\,dV \,.
\Eeq
...
\Beq \label{eq:grid_to_particle}
  \Bf_p \approx \sum_g \Bf_g \phi_{gp} \quad\text{where}\quad
  \phi_{gp} := \frac{\int_{\Omega_p^\star} S_g(\Bx) \omega^\star_p(\Bx)\,dV}{
                       \int_{\Omega_p^\star} \omega^\star_p(\Bx)\,dV} ~,~~
  \omega^\star_p(\Bx) := \omega_p(\Bx) \,\chi^\star_p(\Bx)\,dV \,.
\Eeq
\end{NoteBox}
...
\Beq
  \overbar{S}_{gp} := \phi_{gp} = \frac{\int_{\Omega_p} S_g(\Bx) V_p \delta(\Bx - \Bx_p)\,dV}{
                       \int_{\Omega_p} V_p \delta(\Bx - \Bx_p)\,dV} = S_g(\Bx_p)\,.
\Eeq
The gradient of the interpolation function evaluated at the particle is
\Beq
  \overbar{\BGv}_{gp} = \frac{\int_{\Omega_p} \Grad{S_g(\Bx)} V_p \delta(\Bx - \Bx_p)\,dV}{
                       \int_{\Omega_p} V_p \delta(\Bx - \Bx_p)\,dV} = \Grad{S_g(\Bx_p)}\,.
\Eeq
...
\Beq
  \overbar{S}_{gp} := \phi_{gp} = \begin{cases}
                \frac{1}{V^\star_p} \int_{\Omega_p^\star} S_g(\Bx)\,dV & \quad \text{for}~~\Bx \in \Omega_p^\star \\
...
\Beq
  \overbar{\BGv}_{gp} = \begin{cases}
                \frac{1}{V^\star_p} \int_{\Omega_p^\star} \Grad{S_g(\Bx)}\,dV & \quad \text{for}~~\Bx \in \Omega_p^\star \\
                0 & \quad \text{otherwise.}
              \end{cases}
\Eeq
...
\Beq
  \overbar{S}_{gp} := \phi_{gp} = \begin{cases}
                \frac{1}{V^\star_p} \int_{\Omega_p^\star} S_g^\star(\Bx)\,dV & \quad \text{for}~~\Bx \in \Omega_p^\star \\
                0 & \quad \text{otherwise.}
              \end{cases}
\Eeq
The gradient of the interpolation function is
\Beq
  \overbar{\BGv}_{gp} = \begin{cases}
                \frac{1}{V^\star_p} \int_{\Omega_p^\star} \Grad{S_g^\star(\Bx)}\,dV & \quad \text{for}~~\Bx \in \Omega_p^\star \\
                0 & \quad \text{otherwise.}
              \end{cases}
\Eeq
...
For the particle-to-grid projection (see \eqref{eq:particle_to_grid}), consider the
case where $w(\Bx) = \rho(\Bx)$ where $\rho$ is the mass density.  Then,
\Beq
  A_g = \IntOmega \rho(\Bx) S_g(\Bx) \,dV = m_g ~,~~
  B_{gp} = \IntOmega \rho(\Bx) S_g(\Bx) \chi_p(\Bx) \,dV
\Eeq
and
\Beq \label{eq:mass_weighted}
  \boxed{
  m_g \Bf_g = \sum_p \Bf_p \IntOmega \rho(x) S_g(x) \chi_p(\Bx) \,dV 
...
\Beq
  A_g = \IntOmega  S_g(\Bx) \,dV = V_g ~,~~
  B_{gp} = \IntOmega S_g(\Bx) \chi_p(\Bx) \,dV 
\Eeq
and
\Beq \label{eq:vol_weighted}
  \boxed{
  V_g \Bf_g = \sum_p \Bf_p \IntOmega S_g(x) \chi_p(\Bx) \,dV 
...
\Beq \label{MPM:acceleration}
  \Ba_g = \frac{\Bf_g^{\Text} - \Bf_g^{\Tint}}{m_g}
\Eeq
