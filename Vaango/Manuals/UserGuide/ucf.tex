...
\lstinline!num_processors! is the number of processors that will
...
found in \lstinline!src/StandAlone/inputs/UPS_SPEC/ups_spec.xml!
...
\begin{itemize}
\item \lstinline!maxTime! - how long in physical time to run the simulation for
\item \lstinline!initTime! - what time to begin the simulation at
\item \lstinline!delt_min! - the smallest timestep the simulation will take
\item \lstinline!delt_max! - the largest timestep the simulation will take
\item \lstinline!timestep_multiplier! - multiplies the timestep by this number (before adjusting to min or max timestep)
\end{itemize}

The following fields are optional:

\begin{itemize}
\item \lstinline!delt_init! - The timestep to take initially (assuming it's less than the one computed by the simulation)
\item \lstinline!initial_delt_range! - The period of time to use the \lstinline!delt_init! (default = 0)
\item \lstinline!max_delt_increase! - Maximum amount to multiply the previous delt by (if the newly computed delt is greater than the previous one)
\item \lstinline!max_iterations! - The number of timesteps to run the simulation for (even on a restart)
\item \lstinline!max_Timesteps! - The timestep number to end the simulation on (not usually used with \lstinline!max_iterations!)
\item \lstinline!override_restart_delt! - On a restart, use this delt instead of the most-recently-used delt.
\item \lstinline!clamp_timesteps_to_output! - Sync the delt with the DataArchiver - when an output timestep occurs, reduce the delt to have the time land on the timestep interval (default = false)
\item \lstinline!end_on_max_time_exactly! - clamp the delt such that the last timesteps end on what was specified in maxTime (default = false)
\end{itemize}

A word about timesteps: In general, the timestep (delt) is computed at various stages within a timestep, and the smallest one is used, unless it needs to raise the delt to the \lstinline!delt_min!.
...
intervals;\\
\lstinline!<outputTimestepInterval> integer_number_of_steps </outputTimestepInterval>!\\
or timestep intervals; \\
\lstinline!<outputInterval> floating_point_time_increment </outputInterval>!\\
...
\end{lstlisting}
where \lstinline!component! is, e.g., \lstinline!mpm!, \lstinline!ice!, etc.

Check-pointing information can be created that provides a mechanism for
restarting a simulation at a later point in time.  The \lstinline!<checkpoint>!
tag with the \lstinline!cycle! and \lstinline! interval! attributes describe how many
copies of checkpoint data is stored (cycle) and how often it is generated
(interval).  You may also use the \lstinline!walltimeStart! and \lstinline!walltimeInterval!
...
\lstinline!inputs/UPS_SPEC/mpm_spec.xml!  is given in the Uintah
Developers Manual.
...
concentric rings of particles.  Particle density is specified by \lstinline!num_axial! and \lstinline!num_radial!, the number of
particles in the axial and radial dimensions, respectively
\normalfont.  Note that these particle density specifications
supercede those specified in the \lstinline!<geom_object> <res>!
tag, which is ignored.

The required discretization scheme may be either \lstinline!pie_slices!
 or \lstinline!constant_particle_volumes!.  For the
\lstinline!pie_slices! discretization, \lstinline!num_angular!  is required to specify the number of particles between \lstinline!arc_start! and \lstinline!arc_angle!.  For the \lstinline!constant_particle_volumes! discretization, the number of
particles between \lstinline!arc_start! and \lstinline!arc_angle!
...
End caps may be added to the smoothcyl using the optional \lstinline!<endcap_thickness>! tag, which specifies the axial
...
geometries within a \lstinline!<geom_object>! tag were not
...
triangulated surface (\lstinline!file_name.tri!) contains a list of integers
describing the connectivity of points specified in \lstinline!file_name.pts!.
...
An additional input in the \lstinline!<geom_object>! field is the
...
level.  The primary regridder used in Uintah is the \lstinline!Tiled! regridder.
...
The \lstinline!max_levels! tag specifies the maximum number of levels
to be created.  The \lstinline!cell_refinement_ratio! tag specifies the 
...
The \lstinline!cell_stability_dilation! tag specifies how many cells around
the refinement flags are also guaranteed to be refined.  The
\lstinline!min_boundary_cells! tag specifies the size of the boundary layers. 
The size of the tiles is specified using the \lstinline!min_patch_size! tag
...
\item \tt.dat \normalfont files contain global information about the
  simulation (each line in the .dat files contains: \lstinline!simulation_time!
  value).
...
\begin{wrapfigure}{R}{.50\textwidth}
  \centering
  \includegraphics[width=.40\textwidth]{VisItFileOpen.png}
...
\begin{minipage}[t]{\textwidth}
  ...
  \begin{wrapfigure}{R}{0.5\textwidth}
    \centering
    \includegraphics[width=.40\textwidth]{VisItPlots.png}
...
\begin{wrapfigure}{R}{0.5\textwidth}
  \centering
  \includegraphics[width=.45\textwidth]{VisItVolumeAtts.png}
...
\begin{figure}[t!]
  ...
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSelectPlot.png}
    \caption{Applying the Pseudocolor plot to a variable}
    \label{VisItSelectPlot}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSelectOperator.png}
...
\begin{figure}
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItOrderOperator.png}
    \caption{Ordering of an operator and a plot}
    \label{VisItOrderOperator}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\textwidth}
  \includegraphics[width=\textwidth]{VisItSliceAtts.png}
...
\begin{figure}
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\linewidth]{VisItSubsetIcon.png}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItVolumeTempINATK08Viewer.png}
    \caption{Visualization of a volume on the viewer window}
    \label{VisItVolumeTempINATK08Viewer}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItVolumeAttributes1.png}
    \caption{Volume visualization attributes window}
    \label{VisItVolumeAttributes1}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItVolumeAttributes2.png}
    \caption{Volume visualization attributes window}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItMoleculePTempATK08.png}
    \caption{Selecting a molecule plot and an associated variable/ material}
    \label{VisItMoleculePTempATK08}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItMoleculeAttributesPTemp1.png}
    \caption{Selecting a molecule plot and an associated variable/ material}
    \label{VisItMoleculeAttributesPTemp1}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItMoleculePTtempViewer.png}
    \caption{Selecting a molecule plot and an associated variable/ material}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSubsetPlotVariables.png}
    \caption{A patch/ level variable, associated with every kind of variable}
    \label{VisItSubsetPlotVariables}
  \hspace{50pt}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSubsetPlotViewer.png}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSubsetPlotAttrib.png}
    \caption{Enabling the 'Wireframe' mode for visualizing patch boundaries}
    \label{VisItSubsetPlotAttrib}
  \hspace{50pt}
  \end{subfigure}
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{VisItSubsetPlotWireframe.png}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItContourPlotViewer.png}
    \caption{Iso-surface visualization}
    \label{VisItContourPlotViewer}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
   \includegraphics[width=\textwidth]{VisItContourPlotAttrib.png}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItStreamlinesAttrib1.png}
    \caption{Setting the 'Source' tab parameters}
    \label{VisItStreamlinesAttrib1}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItStreamlinesAttrib2.png}
    \caption{Setting the 'Advanced' tab parameters}
    \label{VisItStreamlinesAttrib2}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItStreamlinesViewer.png}
...
\begin{figure}[htb]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItInverseGhostZoneOp.png}
    \caption{Selecting the Inverse Ghost Zone operator}
    \label{VisItInverseGhostZoneOp}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItInverseGhostZoneAttribs.png}
    \caption{The attributes window for the 'Inverse Ghost Zone' operator}
    \label{VisItInverseGhostZoneAttribs}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[width=\textwidth]{VisItInverseGhostZoneViewer.png}
