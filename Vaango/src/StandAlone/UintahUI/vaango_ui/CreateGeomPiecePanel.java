...
public class CreateGeomPiecePanel extends JPanel 
  ...
  public void createSolidPartListGeomPiece(String simComponent) {
    ...
    if (partType == Particle.CIRCLE) {
      createSolidCylinderGeomPiece();
    } else if (partType == Particle.SPHERE){
      createSolidSphereGeomPiece(); 
    } else {
      System.out.println("Unknown solid particle type in particle list."+"" +
          " No geometry pieces created.");
    }
  }
  ...
  public void createHollowPartListGeomPiece(String simComponent) {
    ...
    int partType = d_partList.getParticle(0).getType();
    if (partType == Particle.CIRCLE) {
      createHollowCylinderGeomPiece();
    } else if (partType == Particle.SPHERE) {
      createHollowSphereGeomPiece();
    } else {
      System.out.println("Unknown hollow particle type in particle list."+"" +
          " No geometry pieces created.");
    }
  }
  
  // Create solid cylinder geometry piece
  private void createSolidCylinderGeomPiece() {
  ...
  // Create solid sphere geometry piece
  private void createSolidSphereGeomPiece() {
    ...
    for (int ii = 0; ii < numPart; ++ii) {
      ...
      // Get the center, radius, and length
      Point center = part.getCenter();
      double outer_radius = part.getRadius();
      double inner_radius = 0.0;
      
      // Estimate the number of material points in the radial direction
      int numRadial = (int) Math.ceil(outer_radius/pointSpacing);

      // Create a name 
      String name = new String("solid_sphere_"+String.valueOf(ii));

      // Create a smooth cylinder geometry piece
      SmoothSphereGeomPiece piece = 
          new SmoothSphereGeomPiece(name, center, outer_radius, inner_radius, 
              numRadial); 
      d_geomPiece.addElement(piece);

      // Create a cylinder geometry piece
      String solidName = new String("outer_sphere_"+String.valueOf(ii));
      SphereGeomPiece sphPiece = 
          new SphereGeomPiece(solidName, center, outer_radius); 
  ...
  // Create hollow cylinder geometry pieces
  private void createHollowCylinderGeomPiece() {
  ...
  // Create hollow sphere geometry pieces
  private void createHollowSphereGeomPiece() {
    ...
    for (int ii = 0; ii < numPart; ++ii) {
      ...
      SmoothSphereGeomPiece piece = 
          new SmoothSphereGeomPiece(name, center, outer_radius, inner_radius, numRadial); 
      d_geomPiece.addElement(piece);

      // Create a sphere geometry piece for the full cylinder
      String solidName = new String("outer_sphere_"+String.valueOf(ii));
      SphereGeomPiece sphPieceSolid = 
          new SphereGeomPiece(solidName, center, outer_radius); 
      unionOuter.addGeomPiece(sphPieceSolid);

      // Create a sphere geometry piece for the inner hollow region
      // of each sphere
      String hollowName = new String("inner_sphere_"+String.valueOf(ii));
      SphereGeomPiece sphPieceHollow = 
          new SphereGeomPiece(hollowName, center, inner_radius); 
