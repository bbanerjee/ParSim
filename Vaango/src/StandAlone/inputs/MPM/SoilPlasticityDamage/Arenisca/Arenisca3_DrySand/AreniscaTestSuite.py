...
default_inputs_path = uintah_src_path + \
    '/StandAlone/inputs/MPM/Arenisca/Arenisca3_DrySand/'
default_working_dir = uintah_src_path + \
    '/StandAlone/inputs/MPM/Arenisca/Arenisca3_DrySand/test_run'
  ...
  copys deformation file to same root folder.'''
    ...
    for line in from_file_lines:
        if '<prescribed_deformation_file>' in line and '</prescribed_deformation_file>' in line:
            ...
            line = line.replace(def_file, def_file.split(
                'inputs/MPM/Arenisca/')[1])
            def_file = def_file.split('inputs/MPM/Arenisca/')[1]
            shutil.copyfile(default_inputs_path+'/'+def_file,
                            to_file_root+'/'+def_file)
...
def run_test(ups_path, WITH_MPI=False, NUM_PROCS=1, RESTART=False, DAMPING_OFF_NEW_END_TIME=False,
    ...
    if not (WITH_MPI) or int(NUM_PROCS) <= 1:
        ...
        args = ['mpirun', '-np', str(int(NUM_PROCS)),
                uintah_exe, '-mpi', os.path.split(ups_path)[1]]
    ...
    if POST_PROC_ONLY:
        ...
        if RESTART:
            ...
            if DAMPING_OFF_NEW_END_TIME:
                ...
                print 'Done.\nRestarting...'
                # Open new runlog
                F_log = open(root_path+'/TEST_RUNLOG_RESTART_' +
                             os.split(ups_path)[1], "w")
                # Construct the argument list
                if not (WITH_MPI) or NUM_PROCS <= 1:
                    args = [uintah_exe, '-restart', '-move', uda_path+'.000']
                else:
                    args = ['mpirun', '-np', str(int(NUM_PROCS)), uintah_exe,
                            '-mpi', '-restart', '-move', uda_path+'.000']
                # Run the test and wait for it to complete
                tmp = sub_proc.Popen(args, stdout=F_log, stderr=sub_proc.PIPE)
                dummy = tmp.wait()
                F_log.close()
...
def clear_uda(uda_path):
    ...
    tmp = sub_proc.Popen(['rm', '-rf', uda_path],
                         stdout=sub_proc.PIPE, stderr=sub_proc.PIPE)
    dummy = tmp.wait()
...
def run_all_tests(TEST_METHODS=False, CLEAR_UDA=False, POST_PROC_ONLY=False):
    ...
    for test in TEST_LIST:
        ...
        ups_path = copy_test_to(
            test, default_working_dir+'/'+os.path.split(test)[1])
        # Run
        if test not in RESTART_LIST[0]:
            uda_path = run_test(ups_path, WITH_MPI=MPI_FLAG,
                                NUM_PROCS=NUM_CPUS, POST_PROC_ONLY=POST_PROC_ONLY)
        else:
            new_end_time = RESTART_LIST[1][RESTART_LIST[0].index(test)]
            uda_path = run_test(ups_path, WITH_MPI=MPI_FLAG, NUM_PROCS=NUM_CPUS,
                                DAMPING_OFF_NEW_END_TIME=new_end_time, POST_PROC_ONLY=POST_PROC_ONLY)
