...
def selectpart(select_exe, work_dir, uda_name, 
  ...
  box = selection_box
  command = (select_exe + " " +
             "-line " +  str(box.xmin) + " " + str(box.ymin) + " " + str(box.zmin) + " " +
             str(box.xmax) + " " + str(box.ymax) + " " + str(box.zmax) + " " + 
             "-mat " + str(material_id) + " " +  
             "-timesteplow " + str(timestep_low) + " " +
             "-timestephigh " +  str(timestep_high) + " " +
             "-uda " + uda_dir)
  print(command)
...
def extractPos(extract_exe, work_dir, uda_name, plot_dir, material_id, time_id,
  ...
  command = (extract_exe + " -m " + str(material_id) + " -p " + part_list_file +
             " -timestep " + str(time_id) + " -uda " + uda_dir + " -o dummy ")
  print(command)
  args_pos = shlex.split(command)
  print args_pos
