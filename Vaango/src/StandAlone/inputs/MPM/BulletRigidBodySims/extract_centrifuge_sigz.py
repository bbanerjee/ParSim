...
def main(argv):
  ...
  parser = argparse.ArgumentParser(
    usage="extract_centrifuge_sigz [--x|--y] --point <x> <y> <z> --dx <value> --mat <mat id> --uda <uda file_name_prefix>")
...
def selectpart(select_exe, work_dir, uda_name, 
  ...
  box = selection_box
  command = (select_exe + 
             " -point " +  str(box.xmin) + " " +  str(box.ymin) + " " +  str(box.zmin) + 
...
#--------------------------------------------------------------------------
def plot_pressure(fig, press_data, location, plt_color, line_style):
  ...
  plt.ylabel(str_to_mathbf('Vertical stress (kPa)'))
...
if __name__ == "__main__":
  ...
  for idx, pos in enumerate(positions):
    ...
    rad = np.sqrt((x-x0)*(x-x0) + (y-y0)*(y-y0))
  ...
  plt_file = os.path.join(plot_dir, uda_name+".sigz")
