...
class fvar {
 ...
 public:
  ...
  template <typename T, typename = typename boost::enable_if<boost::is_arithmetic<std::decay_t<T>>>::type>
...
fvar<RealType, Order> lambert_w0(fvar<RealType, Order> const& cr) {
  using std::exp;
  using ::autodiff_v1::detail::lambert_w0;
  using root_type = typename fvar<RealType, Order>::root_type;
  constexpr size_t order = fvar<RealType, Order>::order_sum;
  root_type derivatives[order + 1];
...
template <typename RealType, size_t Order>
class numeric_limits<autodiff_v1::detail::fvar<RealType, Order>>
    : public numeric_limits<typename autodiff_v1::detail::fvar<RealType, Order>::root_type> {
};
...
using autodiff_fvar_type = autodiff_v1::detail::fvar<RealType, Order>;
...
struct promote_args_2<autodiff_v1::detail::autodiff_fvar_type<RealType0, Order0>,
                      autodiff_v1::detail::autodiff_fvar_type<RealType1, Order1>> {
  using type = autodiff_v1::detail::autodiff_fvar_type<typename promote_args_2<RealType0, RealType1>::type,
...
template <typename RealType, size_t Order>
struct promote_args<autodiff_v1::detail::autodiff_fvar_type<RealType, Order>> {
  using type = autodiff_v1::detail::autodiff_fvar_type<typename promote_args<RealType>::type, Order>;
};

template <typename RealType0, size_t Order0, typename RealType1>
struct promote_args_2<autodiff_v1::detail::autodiff_fvar_type<RealType0, Order0>, RealType1> {
  using type = autodiff_v1::detail::autodiff_fvar_type<typename promote_args_2<RealType0, RealType1>::type, Order0>;
};

template <typename RealType0, typename RealType1, size_t Order1>
struct promote_args_2<RealType0, autodiff_v1::detail::autodiff_fvar_type<RealType1, Order1>> {
  using type = autodiff_v1::detail::autodiff_fvar_type<typename promote_args_2<RealType0, RealType1>::type, Order1>;
};

template <typename destination_t, typename RealType, std::size_t Order>
inline BOOST_MATH_CONSTEXPR destination_t real_cast(autodiff_v1::detail::autodiff_fvar_type<RealType, Order> const& from_v)
    BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(destination_t) && BOOST_MATH_IS_FLOAT(RealType)) {
  return real_cast<destination_t>(static_cast<autodiff_v1::detail::autodiff_root_type<RealType, Order>>(from_v));
}
...
using fvar_t = autodiff_v1::detail::fvar<Policy, Order>;
template <class Policy, std::size_t Order>
