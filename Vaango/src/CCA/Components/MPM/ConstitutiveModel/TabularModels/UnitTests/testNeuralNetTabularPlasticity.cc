...
public:
  ...
  createInput()
  {
    ...
    xmlNewChild(meta,
                nullptr,
                BAD_CAST "title",
                BAD_CAST "Unit test neural net Tabular Plasticity");
    ...
    xmlNewChild(da,
                nullptr,
                BAD_CAST "filebase",
                BAD_CAST "UniaxialStrainRotateNNTabularPlasticity.uda");
    ...
    std::string table_elastic =
      std::string(currPath) + "/" + "mlp_regression_keras_total_scaled.h5";
    auto elastic =
      xmlNewChild(cm, nullptr, BAD_CAST "elastic_moduli_model", BAD_CAST "");
    xmlNewProp(elastic, BAD_CAST "type", BAD_CAST "neural_net");
    xmlNewChild(
      elastic, nullptr, BAD_CAST "filename", BAD_CAST table_elastic.c_str());
    xmlNewChild(elastic, nullptr, BAD_CAST "min_strain", BAD_CAST "0.0");
    xmlNewChild(elastic, nullptr, BAD_CAST "max_strain", BAD_CAST "0.452");
    xmlNewChild(elastic, nullptr, BAD_CAST "min_pressure", BAD_CAST "0.0");
    xmlNewChild(elastic, nullptr, BAD_CAST "max_pressure", BAD_CAST "1.0e6");
    ...
    std::string ups_file = std::string(currPath) + "/" +
                           "UniaxialStrainRotateNNTabularPlasticity.ups";
...
TEST(NeuralNetTabularPlasticityTest, singleParticleTest)
{
  char currPath[2000];
  if (!getcwd(currPath, sizeof(currPath))) {
    std::cout << "Current path not found\n";
  ...
  std::string ups_file =
    std::string(currPath) + "/" + "UniaxialStrainRotateNNTabularPlasticity.ups";
  // std::cout << "Created Filename = " << ups_file << std::endl;

  // Remove existing uda
  std::string uda_file = std::string(currPath) + "/" +
                         "UniaxialStrainRotateNNTabularPlasticity.uda.000";
