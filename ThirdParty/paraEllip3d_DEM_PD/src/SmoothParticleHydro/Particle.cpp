...
Particle::update()
{
  ...
  if (getType() == 0 ||
    ...
    currPos = prevPos + currVeloc * timeStep;
  } else if (getType() == 4) { // special case 4 (impacting ellipsoidal
                               // penetrator): impact with inital velocity in
...
void
Particle::setDemParticleInSPHParticle()
{
  for (std::vector<sph::SPHParticle*>::iterator it =
         SPHGhostParticleVec.begin();
       it != SPHGhostParticleVec.end(); it++) {
    (*it)->setDemParticle(this); // this is the self pointer
  }
}

void
Particle::setNULLDemParticleInSPHParticle()
{
  for (std::vector<sph::SPHParticle*>::iterator it =
         SPHGhostParticleVec.begin();
       it != SPHGhostParticleVec.end(); it++) {
