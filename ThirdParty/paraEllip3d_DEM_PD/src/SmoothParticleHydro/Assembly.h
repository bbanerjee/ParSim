...
private:
  ...
  REAL Wqmin;  // the parameters used to remove tensile instability in
               // "State-of-the-art of classical SPH for free-surface flows"
  REAL p1, p2; // for the Lennard-Jones boundary forces
  REAL sphCellSize; // the cell size of sph domain, sphCellSize = kernelSize in
                    // burstingDam3D, while sphCellSize =
                    // gradation.getMaxRadius()*2
  ...
  //  for sph
  std::vector<sph::SPHParticle*> d_allSPHParticleVec; // this contains all sph
                                                      // particles except ghost
                                                      // particles, i.e. free
                                                      // and boundary for all
                                                      // cpus
  std::vector<sph::SPHParticle*> SPHParticleVec;      // this contains all sph
                                                      // particles except ghost
                                                      // particles for each cpu
  std::vector<sph::SPHParticle *> rsphParticleX1,
    rsphParticleX2; // r stands for received
  std::vector<sph::SPHParticle *> rsphParticleY1, rsphParticleY2;
  std::vector<sph::SPHParticle *> rsphParticleZ1, rsphParticleZ2;
  std::vector<sph::SPHParticle *> rsphParticleX1Y1, rsphParticleX1Y2,
    rsphParticleX1Z1, rsphParticleX1Z2;
  std::vector<sph::SPHParticle *> rsphParticleX2Y1, rsphParticleX2Y2,
    rsphParticleX2Z1, rsphParticleX2Z2;
  std::vector<sph::SPHParticle *> rsphParticleY1Z1, rsphParticleY1Z2,
    rsphParticleY2Z1, rsphParticleY2Z2;
  std::vector<sph::SPHParticle *> rsphParticleX1Y1Z1, rsphParticleX1Y1Z2,
    rsphParticleX1Y2Z1, rsphParticleX1Y2Z2;
  std::vector<sph::SPHParticle *> rsphParticleX2Y1Z1, rsphParticleX2Y1Z2,
    rsphParticleX2Y2Z1, rsphParticleX2Y2Z2;
  std::vector<sph::SPHParticle*> d_recvSPHParticleVec; // received sph particles
                                                       // (free and boundary)
                                                       // per process
  std::vector<sph::SPHParticle*>
    mergeSPHParticleVec; // merged sph particles (free and boundary) per process
  std::vector<std::vector<sph::SPHParticle*>> SPHParticleCellVec; // a vector to
                                                                  // store the
                                                                  // cell of SPH
                                                                  // partiles
                                                                  // (free,
...
public:
  ...
  ~Assembly()
  {
    ...
    for (std::vector<Particle*>::iterator it = allParticleVec.begin();
      ...
      for (std::vector<sph::SPHParticle*>::iterator st =
        ...
        delete (
          *st); // this is important to free the memories of sph ghost particles
      }
    ...
    for (std::vector<sph::SPHParticle*>::iterator it = SPHParticleVec.begin();
         it != SPHParticleVec.end(); ++it)
      delete (*it);
  ...
  void
  generateSPHParticleNoBottom3D(); // not generate bottom boundary sph particles
  void generateSPHParticleMiddleLayers3D(); // not generate bottom boundary sph
                                            // particles
  ...
  void drainageMiddleLayers(); // fixed some layers of DEM particles in the
  ...
  REAL kernelFunction(const dem::Vec& a, const dem::Vec& b); // kernel function,
                                                             // vec is the rab,
                                                             // h is smoothing
                                                             // length
  REAL kernelFunction(
    REAL s); // kernel function, s is rab/h, h is smoothing length
  REAL partialKernelFunction(
    const dem::Vec& a,
    const dem::Vec& b); // to calculate partial differential dWab_dra
  dem::Vec gradientKernelFunction(const dem::Vec& a,
                                  const dem::Vec& b); // to calculate
                                                      // delta_aWab, where a is
                                                      // the position of the
                                                      // first particle
  ...
  void springForce();
  void
  boundaryForce(); // calcualte forces between rigid boundaries and particles
  ...
  void printParticle(const char* str, std::vector<Particle*>& particleVec)
    const;                                      // print particles info
...
public:
  ...
  void findSPHParticleInRectangle(
