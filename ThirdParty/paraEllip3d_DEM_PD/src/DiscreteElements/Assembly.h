...
class Assembly {
 private:
  ...
  // container property
  Rectangle
      allContainer;     // whole container, broadcast among processes for once
  ...
  periDynamics::PeriParticlePArray allPeriParticleVecInitial;  // only for master cpu
  ...
  periDynamics::PeriParticlePArray fixedPeriParticleVec;  // this is the peri-points that are
                                            // inside the rigid
                                            // inclusion, fix these peri-points
  periDynamics::PeriParticlePArray outerfacePeriParticleVec;  // this is only for the hollow
                                                // spherical example
  ...
  periDynamics::PeriParticlePArray rperiParticleX1, rperiParticleX2;  // r stands for received
  ...
  periDynamics::PeriParticlePArray rperiParticleX1Y1, rperiParticleX1Y2, rperiParticleX1Z1,
      rperiParticleX1Z2;
  periDynamics::PeriParticlePArray rperiParticleX2Y1, rperiParticleX2Y2, rperiParticleX2Z1,
      rperiParticleX2Z2;
  periDynamics::PeriParticlePArray rperiParticleY1Z1, rperiParticleY1Z2, rperiParticleY2Z1,
      rperiParticleY2Z2;
  ...
  periDynamics::PeriParticlePArray recvPeriParticleVec;  // received particles per process
  ...
  periDynamics::PeriBondPArray periBondVec;  // peri-bonds between periParticleVec in each cpu
 ...
 public:
  ...
  void planeStrain();
  void trueTriaxial();
  bool tractionErrorTol(REAL sigma,
                        std::string type,
                        REAL sigmaX = 0,
                        REAL sigmaY = 0);
  ...
  void printCompressProg(std::ofstream& ofs,
                         REAL distX,
                         REAL distY,
                         REAL distZ);
  ...
  void trimCavity(bool toRebuild,
  ...
  void writeMeshCheckVolume(
      const char*);  // writeMeshCheckVolume - outputs the mesh and volume, will
                     ...
                     // @param char * - reference of the output file name
  void writeParticleTecplot(std::ofstream&,
                            const int)
      const;  // writeParticleTecplot - outputs the information of all
 ...
 public:
  ...
  void findPeriParticleInRectangle(const Rectangle& container,
                                   const periDynamics::PeriParticlePArray& allParticle,
