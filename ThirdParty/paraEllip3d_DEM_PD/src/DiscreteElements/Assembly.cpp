...
long int timediffmsec(const struct timeval& time1,
                      const struct timeval& time2) {
...
void Assembly::tuneMassPercentage() {
  if (mpiRank == 0) {
    ...
    for (auto itr = allParticleVec.cbegin(); itr != allParticleVec.cend();
         ++itr)
...
void Assembly::deposit(const char* inputBoundary, const char* inputParticle) {
  if (mpiRank == 0) {
  ...
  char cstr0[50];
  /**/ REAL timeCount = 0;
  /**/ timeAccrued = static_cast<REAL>(
      dem::Parameter::getSingleton().parameter["timeAccrued"]);
...
void Assembly::trueTriaxial() {
  ...
  if (mpiRank == 0) {
    plotBoundary(
        strcat(combineString(cstr0, "trueTriaxial_bdryplot_", iterSnap - 1, 3),
               ".dat"));
    plotGrid(
        strcat(combineString(cstr0, "trueTriaxial_gridplot_", iterSnap - 1, 3),
               ".dat"));
    printParticle(
  ...
  while (iteration <= endStep) {
    ...
    if (iteration % (netStep / netSnap) == 0) {
      ...
      if (mpiRank == 0) {
        plotBoundary(
            strcat(combineString(cstr, "trueTriaxial_bdryplot_", iterSnap, 3),
                   ".dat"));
        plotGrid(
            strcat(combineString(cstr, "trueTriaxial_gridplot_", iterSnap, 3),
                   ".dat"));
        printParticle(
...
bool Assembly::tractionErrorTol(REAL sigma,
                                std::string type,
                                REAL sigmaX,
                                REAL sigmaY) {
  ...
  */
  while (timeAccrued < timeTotal) {
    ...
    if (timeCount >= timeTotal / netSnap) {
      ...
      if (mpiRank == 0) {
        plotBoundary(strcat(
            combineString(cstr, "couple_bdryplot_", iterSnap, 3), ".dat"));
        plotGrid(strcat(combineString(cstr, "couple_gridplot_", iterSnap, 3),
                        ".dat"));
...
void Assembly::findPeriParticleInRectangle(
...
void Assembly::removeParticleOutRectangle() {
  ...
  // members is not needed
  REAL epsilon = EPS;
  particleVec.erase(std::remove_if(particleVec.begin(), particleVec.end(),
                                   [&x1, &y1, &z1, &x2, &y2, &z2,
                                    &epsilon](ParticleP particle) {
                                     Vec center = particle->getCurrPos();
  ...
  periParticleVec.erase(
      std::remove_if(
          ...
          [&x1, &y1, &z1, &x2, &y2, &z2, &epsilon](PeriParticleP particle) {
            Vec center = particle->getCurrPosition();
            if (!(center.getX() - x1 >= -epsilon &&
                  center.getX() - x2 < -epsilon &&
                  center.getY() - y1 >= -epsilon &&
                  center.getY() - y2 < -epsilon &&
...
void Assembly::scatterParticle() {
  ...
  if (mpiRank == 0) {  // process 0
    ...
    for (int iRank = mpiSize - 1; iRank >= 0; --iRank) {
      ...
      MPI_Cart_coords(cartComm, iRank, ndim, coords);
      Rectangle container(
          v1.getX() + vspan.getX() / mpiProcX * coords[0],
      ...
      if (iRank != 0)
        reqs[iRank - 1] = boostWorld.isend(
            iRank, mpiTag, tmpParticleVec);  // non-blocking send
      if (iRank == 0) {
...
void Assembly::scatterDEMPeriParticle() {
  ...
  if (mpiRank == 0) {  // process 0
    setGrid(
        Rectangle(allContainer.getMinCorner().getX() - point_interval * 0.2,
  ...
  if (mpiRank == 0) {  // process 0
    ...
    for (int iRank = mpiSize - 1; iRank >= 0; --iRank) {
      ...
      Rectangle container(
          ...
          v1.getY() + vspan.getY() / mpiProcY * (coords[1] + 1),
          v1.getZ() + vspan.getZ() / mpiProcZ * (coords[2] + 1));
      findPeriParticleInRectangle(container, allPeriParticleVec,
                                  tmpPeriParticleVec);
      if (iRank != 0)
        reqs[iRank - 1] = boostWorld.isend(
...
void Assembly::commuParticle() {
  ...
  Vec vspan = v2 - v1;
  container =
      Rectangle(v1.getX() + vspan.getX() / mpiProcX * mpiCoords[0],
  ...
  // debugInf << "rank=" << mpiRank << ' ' << v1.getX() << ' ' << v1.getY() << '
  // ' << v1.getZ() << ' '  << v2.getX() << ' ' << v2.getY() << ' ' << v2.getZ()
  // << std::endl;
  REAL cellSize = gradation.getPtclMaxRadius() * 2;
  // 6 surfaces
  if (rankX1 >= 0) {  // surface x1
    Rectangle containerX1(v1.getX(), v1.getY(), v1.getZ(), v1.getX() + cellSize,
                          v2.getY(), v2.getZ());
  ...
  if (rankX2 >= 0) {  // surface x2
    Rectangle containerX2(v2.getX() - cellSize, v1.getY(), v1.getZ(), v2.getX(),
                          v2.getY(), v2.getZ());
  ...
  if (rankY1 >= 0) {  // surface y1
    Rectangle containerY1(v1.getX(), v1.getY(), v1.getZ(), v2.getX(),
                          v1.getY() + cellSize, v2.getZ());
  ...
  if (rankY2 >= 0) {  // surface y2
    Rectangle containerY2(v1.getX(), v2.getY() - cellSize, v1.getZ(), v2.getX(),
                          v2.getY(), v2.getZ());
  ...
  if (rankZ1 >= 0) {  // surface z1
    Rectangle containerZ1(v1.getX(), v1.getY(), v1.getZ(), v2.getX(), v2.getY(),
                          v1.getZ() + cellSize);
  ...
  if (rankZ2 >= 0) {  // surface z2
    Rectangle containerZ2(v1.getX(), v1.getY(), v2.getZ() - cellSize, v2.getX(),
                          v2.getY(), v2.getZ());
  ...
  if (rankX1Y1 >= 0) {  // edge x1y1
    Rectangle containerX1Y1(v1.getX(), v1.getY(), v1.getZ(),
                            v1.getX() + cellSize, v1.getY() + cellSize,
                            v2.getZ());
  ...
  if (rankX2Y2 >= 0) {  // edge x2y2
    Rectangle containerX2Y2(v2.getX() - cellSize, v2.getY() - cellSize,
                            v1.getZ(), v2.getX(), v2.getY(), v2.getZ());
  ...
  if (rankX2Z2 >= 0) {  // edge x2z2
    Rectangle containerX2Z2(v2.getX() - cellSize, v1.getY(),
                            v2.getZ() - cellSize, v2.getX(), v2.getY(),
                            v2.getZ());
  ...
  if (rankY1Z1 >= 0) {  // edge y1z1
    Rectangle containerY1Z1(v1.getX(), v1.getY(), v1.getZ(), v2.getX(),
                            v1.getY() + cellSize, v1.getZ() + cellSize);
  ...
  if (rankY2Z2 >= 0) {  // edge y2z2
    Rectangle containerY2Z2(v1.getX(), v2.getY() - cellSize,
                            v2.getZ() - cellSize, v2.getX(), v2.getY(),
                            v2.getZ());
  ...
  if (rankX1Y2Z2 >= 0) {  // edge x1y2z2
    Rectangle containerX1Y2Z2(v1.getX(), v2.getY() - cellSize,
                              v2.getZ() - cellSize, v1.getX() + cellSize,
                              v2.getY() + cellSize, v2.getZ());
  ...
  if (rankX2Y1Z2 >= 0) {  // edge x2y1z2
    Rectangle containerX2Y1Z2(v2.getX() - cellSize, v1.getY(),
                              v2.getZ() - cellSize, v2.getX(),
                              v1.getY() + cellSize, v2.getZ());
  ...
  if (rankX2Y2Z1 >= 0) {  // edge x2y2z1
    Rectangle containerX2Y2Z1(v2.getX() - cellSize, v2.getY() - cellSize,
                              v1.getZ(), v2.getX(), v2.getY(),
                              v1.getZ() + cellSize);
  ...
  if (rankX2Y2Z2 >= 0) {  // edge x2y2z2
    Rectangle containerX2Y2Z2(v2.getX() - cellSize, v2.getY() - cellSize,
                              v2.getZ() - cellSize, v2.getX(), v2.getY(),
                              v2.getZ());
...
void Assembly::commuPeriParticle() {
  ...
  if (rankX1Y1 >= 0) {  // edge x1y1
    Rectangle containerX1Y1(v1.getX(), v1.getY(), v1.getZ(),
                            v1.getX() + cellSize, v1.getY() + cellSize,
                            v2.getZ());
    findPeriParticleInRectangle(containerX1Y1, periParticleVec,
                                periParticleX1Y1);
  ...
  if (rankX1Y2 >= 0) {  // edge x1y2
    Rectangle containerX1Y2(v1.getX(), v2.getY() - cellSize, v1.getZ(),
                            v1.getX() + cellSize, v2.getY(), v2.getZ());
    findPeriParticleInRectangle(containerX1Y2, periParticleVec,
                                periParticleX1Y2);
  ...
  if (rankX1Z1 >= 0) {  // edge x1z1
    Rectangle containerX1Z1(v1.getX(), v1.getY(), v1.getZ(),
                            v1.getX() + cellSize, v2.getY(),
                            v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX1Z1, periParticleVec,
                                periParticleX1Z1);
  ...
  if (rankX1Z2 >= 0) {  // edge x1z2
    Rectangle containerX1Z2(v1.getX(), v1.getY(), v2.getZ() - cellSize,
                            v1.getX() + cellSize, v2.getY(), v2.getZ());
    findPeriParticleInRectangle(containerX1Z2, periParticleVec,
                                periParticleX1Z2);
  ...
  if (rankX2Y1 >= 0) {  // edge x2y1
    Rectangle containerX2Y1(v2.getX() - cellSize, v1.getY(), v1.getZ(),
                            v2.getX(), v1.getY() + cellSize, v2.getZ());
    findPeriParticleInRectangle(containerX2Y1, periParticleVec,
                                periParticleX2Y1);
  ...
  if (rankX2Y2 >= 0) {  // edge x2y2
    Rectangle containerX2Y2(v2.getX() - cellSize, v2.getY() - cellSize,
                            v1.getZ(), v2.getX(), v2.getY(), v2.getZ());
    findPeriParticleInRectangle(containerX2Y2, periParticleVec,
                                periParticleX2Y2);
  ...
  if (rankX2Z1 >= 0) {  // edge x2z1
    Rectangle containerX2Z1(v2.getX() - cellSize, v1.getY(), v1.getZ(),
                            v2.getX(), v2.getY(), v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX2Z1, periParticleVec,
                                periParticleX2Z1);
  ...
  if (rankX2Z2 >= 0) {  // edge x2z2
    Rectangle containerX2Z2(v2.getX() - cellSize, v1.getY(),
                            v2.getZ() - cellSize, v2.getX(), v2.getY(),
                            v2.getZ());
    findPeriParticleInRectangle(containerX2Z2, periParticleVec,
                                periParticleX2Z2);
  ...
  if (rankY1Z1 >= 0) {  // edge y1z1
    Rectangle containerY1Z1(v1.getX(), v1.getY(), v1.getZ(), v2.getX(),
                            v1.getY() + cellSize, v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerY1Z1, periParticleVec,
                                periParticleY1Z1);
  ...
  if (rankY1Z2 >= 0) {  // edge y1z2
    Rectangle containerY1Z2(v1.getX(), v1.getY(), v2.getZ() - cellSize,
                            v2.getX(), v1.getY() + cellSize, v2.getZ());
    findPeriParticleInRectangle(containerY1Z2, periParticleVec,
                                periParticleY1Z2);
  ...
  if (rankY2Z1 >= 0) {  // edge y2z1
    Rectangle containerY2Z1(v1.getX(), v2.getY() - cellSize, v1.getZ(),
                            v2.getX(), v2.getY(), v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerY2Z1, periParticleVec,
                                periParticleY2Z1);
  ...
  if (rankY2Z2 >= 0) {  // edge y2z2
    Rectangle containerY2Z2(v1.getX(), v2.getY() - cellSize,
                            v2.getZ() - cellSize, v2.getX(), v2.getY(),
                            v2.getZ());
    findPeriParticleInRectangle(containerY2Z2, periParticleVec,
                                periParticleY2Z2);
  ...
  if (rankX1Y1Z1 >= 0) {  // edge x1y1z1
    Rectangle containerX1Y1Z1(v1.getX(), v1.getY(), v1.getZ(),
                              v1.getX() + cellSize, v1.getY() + cellSize,
                              v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX1Y1Z1, periParticleVec,
                                periParticleX1Y1Z1);
  ...
  if (rankX1Y1Z2 >= 0) {  // edge x1y1z2
    Rectangle containerX1Y1Z2(v1.getX(), v1.getY(), v2.getZ() - cellSize,
                              v1.getX() + cellSize, v1.getY() + cellSize,
                              v2.getZ());
    findPeriParticleInRectangle(containerX1Y1Z2, periParticleVec,
                                periParticleX1Y1Z2);
  ...
  if (rankX1Y2Z1 >= 0) {  // edge x1y2z1
    Rectangle containerX1Y2Z1(v1.getX(), v2.getY() - cellSize, v1.getZ(),
                              v1.getX() + cellSize, v2.getY(),
                              v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX1Y2Z1, periParticleVec,
                                periParticleX1Y2Z1);
  ...
  if (rankX1Y2Z2 >= 0) {  // edge x1y2z2
    Rectangle containerX1Y2Z2(v1.getX(), v2.getY() - cellSize,
                              v2.getZ() - cellSize, v1.getX() + cellSize,
                              v2.getY() + cellSize, v2.getZ());
    findPeriParticleInRectangle(containerX1Y2Z2, periParticleVec,
                                periParticleX1Y2Z2);
  ...
  if (rankX2Y1Z1 >= 0) {  // edge x2y1z1
    Rectangle containerX2Y1Z1(v2.getX() - cellSize, v1.getY(), v1.getZ(),
                              v2.getX(), v1.getY() + cellSize,
                              v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX2Y1Z1, periParticleVec,
                                periParticleX2Y1Z1);
  ...
  if (rankX2Y1Z2 >= 0) {  // edge x2y1z2
    Rectangle containerX2Y1Z2(v2.getX() - cellSize, v1.getY(),
                              v2.getZ() - cellSize, v2.getX(),
                              v1.getY() + cellSize, v2.getZ());
    findPeriParticleInRectangle(containerX2Y1Z2, periParticleVec,
                                periParticleX2Y1Z2);
  ...
  if (rankX2Y2Z1 >= 0) {  // edge x2y2z1
    Rectangle containerX2Y2Z1(v2.getX() - cellSize, v2.getY() - cellSize,
                              v1.getZ(), v2.getX(), v2.getY(),
                              v1.getZ() + cellSize);
    findPeriParticleInRectangle(containerX2Y2Z1, periParticleVec,
                                periParticleX2Y2Z1);
  ...
  if (rankX2Y2Z2 >= 0) {  // edge x2y2z2
    Rectangle containerX2Y2Z2(v2.getX() - cellSize, v2.getY() - cellSize,
                              v2.getZ() - cellSize, v2.getX(), v2.getY(),
                              v2.getZ());
    findPeriParticleInRectangle(containerX2Y2Z2, periParticleVec,
                                periParticleX2Y2Z2);
...
void Assembly::updatePeriGrid() {
  ...
  // no need to broadcast grid as it is updated in each process
  setGrid(Rectangle(minX - point_interval * 0.2, minY - point_interval * 0.2,
...
void Assembly::migrateParticle() {
  ...
  // 6 surfaces
  if (rankX1 >= 0) {  // surface x1
    Rectangle containerX1(v1.getX() - segX, v1.getY(), v1.getZ(), v1.getX(),
                          v2.getY(), v2.getZ());
    findParticleInRectangle(containerX1, particleVec, particleX1);
    reqX1[0] = boostWorld.isend(rankX1, mpiTag, particleX1);
    reqX1[1] = boostWorld.irecv(rankX1, mpiTag, rParticleX1);
  ...
  if (rankX2 >= 0) {  // surface x2
    Rectangle containerX2(v2.getX(), v1.getY(), v1.getZ(), v2.getX() + segX,
                          v2.getY(), v2.getZ());
    findParticleInRectangle(containerX2, particleVec, particleX2);
    reqX2[0] = boostWorld.isend(rankX2, mpiTag, particleX2);
    reqX2[1] = boostWorld.irecv(rankX2, mpiTag, rParticleX2);
  ...
  if (rankY1 >= 0) {  // surface y1
    Rectangle containerY1(v1.getX(), v1.getY() - segY, v1.getZ(), v2.getX(),
                          v1.getY(), v2.getZ());
    findParticleInRectangle(containerY1, particleVec, particleY1);
    reqY1[0] = boostWorld.isend(rankY1, mpiTag, particleY1);
    reqY1[1] = boostWorld.irecv(rankY1, mpiTag, rParticleY1);
  ...
  if (rankY2 >= 0) {  // surface y2
    Rectangle containerY2(v1.getX(), v2.getY(), v1.getZ(), v2.getX(),
                          v2.getY() + segY, v2.getZ());
    findParticleInRectangle(containerY2, particleVec, particleY2);
    reqY2[0] = boostWorld.isend(rankY2, mpiTag, particleY2);
    reqY2[1] = boostWorld.irecv(rankY2, mpiTag, rParticleY2);
  }
  if (rankZ1 >= 0) {  // surface z1
    Rectangle containerZ1(v1.getX(), v1.getY(), v1.getZ() - segZ, v2.getX(),
                          v2.getY(), v1.getZ());
    findParticleInRectangle(containerZ1, particleVec, particleZ1);
    reqZ1[0] = boostWorld.isend(rankZ1, mpiTag, particleZ1);
    reqZ1[1] = boostWorld.irecv(rankZ1, mpiTag, rParticleZ1);
  ...
  if (rankZ2 >= 0) {  // surface z2
    Rectangle containerZ2(v1.getX(), v1.getY(), v2.getZ(), v2.getX(), v2.getY(),
                          v2.getZ() + segZ);
    findParticleInRectangle(containerZ2, particleVec, particleZ2);
    reqZ2[0] = boostWorld.isend(rankZ2, mpiTag, particleZ2);
    reqZ2[1] = boostWorld.irecv(rankZ2, mpiTag, rParticleZ2);
  ...
  if (rankX1Y2 >= 0) {  // edge x1y2
    Rectangle containerX1Y2(v1.getX() - segX, v2.getY(), v1.getZ(), v1.getX(),
                            v2.getY() + segY, v2.getZ());
    findParticleInRectangle(containerX1Y2, particleVec, particleX1Y2);
    reqX1Y2[0] = boostWorld.isend(rankX1Y2, mpiTag, particleX1Y2);
    reqX1Y2[1] = boostWorld.irecv(rankX1Y2, mpiTag, rParticleX1Y2);
  ...
  if (rankX1Z2 >= 0) {  // edge x1z2
    Rectangle containerX1Z2(v1.getX() - segX, v1.getY(), v2.getZ(), v1.getX(),
                            v2.getY(), v2.getZ() + segZ);
    findParticleInRectangle(containerX1Z2, particleVec, particleX1Z2);
    reqX1Z2[0] = boostWorld.isend(rankX1Z2, mpiTag, particleX1Z2);
    reqX1Z2[1] = boostWorld.irecv(rankX1Z2, mpiTag, rParticleX1Z2);
  }
  ...
  if (rankX2Y2 >= 0) {  // edge x2y2
    Rectangle containerX2Y2(v2.getX(), v2.getY(), v1.getZ(), v2.getX() + segX,
                            v2.getY() + segY, v2.getZ());
    findParticleInRectangle(containerX2Y2, particleVec, particleX2Y2);
    reqX2Y2[0] = boostWorld.isend(rankX2Y2, mpiTag, particleX2Y2);
    reqX2Y2[1] = boostWorld.irecv(rankX2Y2, mpiTag, rParticleX2Y2);
  ...
  if (rankX2Z2 >= 0) {  // edge x2z2
    Rectangle containerX2Z2(v2.getX(), v1.getY(), v2.getZ(), v2.getX() + segX,
                            v2.getY(), v2.getZ() + segZ);
    findParticleInRectangle(containerX2Z2, particleVec, particleX2Z2);
    reqX2Z2[0] = boostWorld.isend(rankX2Z2, mpiTag, particleX2Z2);
    reqX2Z2[1] = boostWorld.irecv(rankX2Z2, mpiTag, rParticleX2Z2);
  ...
  if (rankY1Z2 >= 0) {  // edge y1z2
    Rectangle containerY1Z2(v1.getX(), v1.getY() - segY, v2.getZ(), v2.getX(),
                            v1.getY(), v2.getZ() + segZ);
    findParticleInRectangle(containerY1Z2, particleVec, particleY1Z2);
    reqY1Z2[0] = boostWorld.isend(rankY1Z2, mpiTag, particleY1Z2);
    reqY1Z2[1] = boostWorld.irecv(rankY1Z2, mpiTag, rParticleY1Z2);
  }
  if (rankY2Z1 >= 0) {  // edge y2z1
    Rectangle containerY2Z1(v1.getX(), v2.getY(), v1.getZ() - segZ, v2.getX(),
                            v2.getY() + segY, v1.getZ());
    findParticleInRectangle(containerY2Z1, particleVec, particleY2Z1);
    reqY2Z1[0] = boostWorld.isend(rankY2Z1, mpiTag, particleY2Z1);
    reqY2Z1[1] = boostWorld.irecv(rankY2Z1, mpiTag, rParticleY2Z1);
  }
  if (rankY2Z2 >= 0) {  // edge y2z2
    Rectangle containerY2Z2(v1.getX(), v2.getY(), v2.getZ(), v2.getX(),
                            v2.getY() + segY, v2.getZ() + segZ);
    findParticleInRectangle(containerY2Z2, particleVec, particleY2Z2);
    reqY2Z2[0] = boostWorld.isend(rankY2Z2, mpiTag, particleY2Z2);
    reqY2Z2[1] = boostWorld.irecv(rankY2Z2, mpiTag, rParticleY2Z2);
  }
  // 8 vertices
  if (rankX1Y1Z1 >= 0) {  // edge x1y1z1
    Rectangle containerX1Y1Z1(v1.getX() - segX, v1.getY() - segY,
                              v1.getZ() - segZ, v1.getX(), v1.getY(),
                              v1.getZ());
    findParticleInRectangle(containerX1Y1Z1, particleVec, particleX1Y1Z1);
    reqX1Y1Z1[0] = boostWorld.isend(rankX1Y1Z1, mpiTag, particleX1Y1Z1);
    reqX1Y1Z1[1] = boostWorld.irecv(rankX1Y1Z1, mpiTag, rParticleX1Y1Z1);
  ...
  if (rankX1Y2Z2 >= 0) {  // edge x1y2z2
    Rectangle containerX1Y2Z2(v1.getX() - segX, v2.getY(), v2.getZ(), v1.getX(),
                              v2.getY() + segY, v2.getZ() + segZ);
    findParticleInRectangle(containerX1Y2Z2, particleVec, particleX1Y2Z2);
    reqX1Y2Z2[0] = boostWorld.isend(rankX1Y2Z2, mpiTag, particleX1Y2Z2);
    reqX1Y2Z2[1] = boostWorld.irecv(rankX1Y2Z2, mpiTag, rParticleX1Y2Z2);
  ...
  if (rankX2Y2Z2 >= 0) {  // edge x2y2z2
    Rectangle containerX2Y2Z2(v2.getX(), v2.getY(), v2.getZ(), v2.getX() + segX,
                              v2.getY() + segY, v2.getZ() + segZ);
    findParticleInRectangle(containerX2Y2Z2, particleVec, particleX2Y2Z2);
    reqX2Y2Z2[0] = boostWorld.isend(rankX2Y2Z2, mpiTag, particleX2Y2Z2);
    reqX2Y2Z2[1] = boostWorld.irecv(rankX2Y2Z2, mpiTag, rParticleX2Y2Z2);
...
void Assembly::migratePeriParticle() {
  ...
  for (PeriParticlePArray::iterator pit = recvPeriParticleVec.begin();
       pit != recvPeriParticleVec.end(); pit++)
...
void Assembly::gatherPeriParticle() {
  ...
  // each other process
  for (PeriParticlePArray::iterator it = periParticleVec.begin();
       it != periParticleVec.end(); it++) {
...
void Assembly::releaseGatheredPeriParticle() {
  ...
  //for (auto it = allPeriParticleVec.begin(); it != allPeriParticleVec.end(); ++it)
  //  delete (*it);
...
void Assembly::gatherBdryContact() {
  ...
  if (mpiRank == 0) {
    ...
    for (BoundaryPArray::iterator it = mergeBoundaryVec.begin();
...
void Assembly::printCompressProg(std::ofstream& ofs,
                                 REAL distX,
                                 REAL distY,
                                 REAL distZ) {
  REAL x1, x2, y1, y2, z1, z2;
...
void Assembly::printPeriProgress(std::ofstream& ofs, const int iframe) const {
  ...
  for (PeriParticlePArray::const_iterator pt = allPeriParticleVec.begin();
       pt != allPeriParticleVec.end(); pt++) {
    ...
    ofs << std::setw(20)
        << (*pt)->getInitPosition().getX() + (*pt)->getDisplacement().getX()
        << std::setw(20)
        << (*pt)->getInitPosition().getY() + (*pt)->getDisplacement().getY()
        << std::setw(20)
        << (*pt)->getInitPosition().getZ() + (*pt)->getDisplacement().getZ()
...
void Assembly::printPeriProgressHalf(std::ofstream& ofs,
  ...
  */
  ...
  for (std::size_t i = 0; i < particleNum; ++i) {
    ifs >> id >> type >> a >> b >> c >> px >> py >> pz >> dax >> day >> daz >>
        dbx >> dby >> dbz >> dcx >> dcy >> dcz >> vx >> vy >> vz >> omx >>
        omy >> omz >> fx >> fy >> fz >> mx >> my >> mz;
...
void Assembly::printParticle(const char* str,
                             ParticlePArray& particleVec) const {
  std::ofstream ofs(str);
  if (!ofs) {
    debugInf << "stream error: printParticle" << std::endl;
    exit(-1);
...
void Assembly::internalForce() {
  ...
  if (contactVec.size() > 0) {
    for (ContactArray::iterator it = contactVec.begin(); it != contactVec.end();
         ++it)
      it->checkinPrevTgt(
...
void Assembly::updateParticle() {
  for (ParticlePArray::iterator it = particleVec.begin();
       it != particleVec.end(); ++it)
    (*it)->update();
}

void Assembly::updateBoundary(REAL sigma,
                              std::string type,
                              REAL sigmaX,
                              REAL sigmaY) {
  if (mpiRank == 0) {
    ...
    for (BoundaryPArray::iterator it = boundaryVec.begin();
         it != boundaryVec.end(); ++it)
      (*it)->clearContactInfo();
...
void Assembly::printContact(char* str) const {
  ...
  for (ContactArray::const_iterator it = contactVec.begin();
...
REAL Assembly::getMass() const {
  REAL var = 0;
  for (ParticlePArray::const_iterator it = allParticleVec.begin();
...
void Assembly::solve(const char* outputFile) {
  ...
  //        << std::setw(20) << periParticleVec[568]->getDisplacement().getZ()
  << std::endl;
  //        }
  //        if( istep % 200 == 0) {
...
void Assembly::writeDisplacementData(const char* outputFilex,
                                     const char* outputFiley,
                                     const char* outputFilez) {
  /* // not used in the DEM-PD coupling code, i.e. rigidInclusion()
  ...
  //        ofs << std::setw(20) <<
  periParticleVec[node]->getInitPosition().getX()
  //        << std::setw(20) << periParticleVec[node]->getDisplacement().getX()
  << std::endl;
  //    }
      ofs.flush();
      ofs.close();
  ...
  //        ofs << std::setw(20) <<
  periParticleVec[node]->getInitPosition().getY()
  //        << std::setw(20) << periParticleVec[node]->getDisplacement().getY()
  << std::endl;
  //    }
      ofs.flush();
      ofs.close();
  ...
  //        ofs << std::setw(20) <<
  periParticleVec[node]->getInitPosition().getZ()
  //        << std::setw(20) << periParticleVec[node]->getDisplacement().getZ()
  << std::endl;
  //    }
      ofs.flush();
      ofs.close();
...
void Assembly::constructNeighbor() {  // this function should be called after
  ...
  if (periParticleVec.empty())
    return;
  for (PeriParticlePArray::iterator i_nt = periParticleVec.begin();
       i_nt != periParticleVec.end(); i_nt++) {
...
#pragma omp parallel for num_threads(ompThreads) private(i_nt, j_nt) shared( \
  ...
  for (i_nt = 0; i_nt < num - 1; i_nt++) {
    ...
    for (j_nt = i_nt + 1; j_nt < num; j_nt++) {
      ...
      if (ratio <= 2.0) {
        // create bond
        PeriBondP bond_pt = std::make_shared<periDynamics::PeriBond>(tmp_length, 
           periParticleVec[i_nt], periParticleVec[j_nt]);
#pragma omp critical
        {
        ...
        if (ratio < 1.0) {
          bond_pt->setWeight(factor * (2.0 / 3.0 - ratio * ratio +
                                       0.5 * ratio * ratio * ratio));
        } else {
...
void Assembly::findRecvPeriBonds() {  // this function should be called after
  ...
  for (auto i_nt = recvPeriParticleVec.begin();
    ...
    for (auto j_nt = periParticleVec.begin();
         j_nt < periParticleVec.end(); j_nt++) {
  ...
  for (auto i_nt = recvPeriParticleVec.begin();
    ...
    Vec coord0_i = (*i_nt)->getInitPosition();
    REAL horizonSize_i = (*i_nt)->getHorizonSize();
    for (auto j_nt = i_nt + 1;
         j_nt < recvPeriParticleVec.end(); j_nt++) {
...
void Assembly::readPeriDynamicsData(
    const char* InputFile) {  // should only be called by master cpu
  ...
  /* // this fixedPeriParticleVec needs to be found in another way, such as by
  locations
  ...
  */
  ...
  //    // read particle indices for linear elasticity verfication, can be
  //    deleted
...
void Assembly::writeMesh(const char* outputFile) {
  /* // not used in DEM-PD coupling code, i.e. rigidInclustion()
     ...
     periParticleVec[node]->getInitPosition().getX()
          ...
          << std::setw(20) << periParticleVec[node]->getInitPosition().getZ() <<
     std::endl;
      }
      for(int iel = 0; iel < nele; iel++){
          for(int node = 0; node < 8; node++){
...
void Assembly::writeParticleTecplot(std::ofstream& ofs,
                                    const int iframe) const {
  ofs.setf(std::ios::scientific, std::ios::floatfield);
  ofs.precision(10);
  if (iframe == 0) {
...
void Assembly::printPeriDomain(const char* str) const {
  ...
  Matrix Kinv_tmp;
  Matrix deformationG;
  for (PeriParticlePArray::const_iterator pt = periParticleVec.begin();
       pt != periParticleVec.end(); pt++) {
    sigma = (*pt)->getSigma();
    pressure = sigma(1, 1) + sigma(2, 2) + sigma(3, 3);
    vonMisesStress =
...
void Assembly::printRecvPeriDomain(const char* str) const {
  ...
  Matrix Kinv_tmp;
  Matrix deformationG;
  for (PeriParticlePArray::const_iterator pt = recvPeriParticleVec.begin();
       pt != recvPeriParticleVec.end(); pt++) {
    sigma = (*pt)->getSigma();
    pressure = sigma(1, 1) + sigma(2, 2) + sigma(3, 3);
    vonMisesStress =
        sqrt(0.5 * ((sigma(1, 1) - sigma(2, 2)) * (sigma(1, 1) - sigma(2, 2)) +
...
void Assembly::printPeriParticle(const char* str) const {
  ...
  for (PeriParticlePArray::const_iterator pt = allPeriParticleVec.begin();
    ...
    ofs << std::setw(OWID) << id << std::setw(OWID)
        << (*pt)->getDisplacement().getX() << std::setw(OWID)
        << (*pt)->getDisplacement().getY() << std::setw(OWID)
        << (*pt)->getDisplacement().getZ() << std::setw(OWID) << vonMisesStress
        ...
        << std::setw(OWID) << (*pt)->getInitPosition().getX() << std::setw(OWID)
        << (*pt)->getInitPosition().getY() << std::setw(OWID)
        << (*pt)->getInitPosition().getZ() << std::endl;
  }
...
void Assembly::setInitIsv() {
  ...
  for (PeriParticlePArray::iterator pt = allPeriParticleVecInitial.begin();
  ...
  for (int iel = 0; iel < nele; iel++) {
    ...
    */
    ...
    for (int node = 0; node < 8; node++) {
      ...
      xl(1, node + 1) =
          allPeriParticleVecInitial[nposition]->getInitPosition().getX();
      xl(2, node + 1) =
          allPeriParticleVecInitial[nposition]->getInitPosition().getY();
      xl(3, node + 1) =
          allPeriParticleVecInitial[nposition]->getInitPosition().getZ();
...
void Assembly::calcParticleKinv() {  // this should be called after
  ...
  for (PeriParticlePArray::iterator pt = recvPeriParticleVec.begin();
       pt != recvPeriParticleVec.end(); pt++) {
    (*pt)->calcParticleKinv();
  }
...
void Assembly::calcRecvParticleKinv() {  // this should be called after
                                         // commuDEMPeriParticle(), and find
                                         // peri-bonds between communicated
                                         // periParticles
  // Compute the inverse of the shape tensor K
...
void Assembly::removeInsidePeriParticles() {
  ...
  for (PeriParticlePArray::iterator pt = allPeriParticleVecInitial.begin();
    ...
    for (ParticlePArray::iterator dem_pt = allParticleVec.begin();
         dem_pt != allParticleVec.end(); dem_pt++) {
      REAL a =
      ...
      Vec coord_pt_tmp = (*dem_pt)->globalToLocal(
          coord_pt -
          (*dem_pt)
              ->getCurrPos());  // this is important to get local coordinate
  ...
  delete points within radius as 0.095m
          ...
          coord_pt = (*dem_pt)->localVec(coord_pt-(*dem_pt)->getCurrPos());
...
void Assembly::eraseBrokenPeriDEMBonds() {
  ...
  // Better approach may be to use a list if random access of vector
  // members is not needed
  periDEMBondVec.erase(
      std::remove_if(
          periDEMBondVec.begin(), periDEMBondVec.end(),
          [](PeriDEMBondP bond) {
            if (bond->getIsAlive()) {
              return false;
            }
...
#pragma omp parallel for num_threads(ompThreads) private(peri_pt) \
  ...
  for (peri_pt = 0; peri_pt < num; peri_pt++) {
    ...
    for (auto dem_pt = mergeParticleVec.begin();
         dem_pt != mergeParticleVec.end(); dem_pt++) {
      ...
      if (x_peri_de * x_peri_de + y_peri_de * y_peri_de +
        ...
        REAL dd = sqrt(x_peri * x_peri + y_peri * y_peri + z_peri * z_peri) *
                  (1 - kd);
...
//            dem-particle
//            PeriDEMBondP bond_tmp = std::make_shared<PeriDEMBond>(projector_local, 
...
void Assembly::constructBoundarySandPeriBonds() {
  ...
  dem_pt!=ParticleVec.end(); dem_pt++){
          ...
          REAL rb = (*dem_pt)->getB();
          REAL rc = (*dem_pt)->getC();
              xyz_peri = (*dem_pt)->localVec(xyz_peri-(*dem_pt)->getCurrPos());
  // this is very important, since all calculations below for ellipsoid
              REAL x_peri = xyz_peri.getX();                        // are based
...
void Assembly::applyCoupledForces() {
  ...
  */
  ...
  for (PeriDEMBondPArray::iterator bond_pt = periDEMBondVec.begin();
       bond_pt != periDEMBondVec.end(); bond_pt++) {
...
void Assembly::constructPeriMatrix() {  // construct the Matrix members in
                                        // periParticleVec,
...
void Assembly::constructRecvPeriMatrix() {  // construct the Matrix members in
                                            // periParticleVec,
  // since currently the transfer of pointer array in Matrix is not implemented
  // well
  for (PeriParticlePArray::iterator pt = recvPeriParticleVec.begin();
...
void Assembly::findBoundaryPeriParticles() {  // it does not matter since this
  ...
  for (PeriParticlePArray::iterator peri_pt = periParticleVec.begin();
       peri_pt != periParticleVec.end(); peri_pt++) {
...
void Assembly::findFixedPeriParticles() {  // it does not matter since this will
                                           // be only called once
  fixedPeriParticleVec.clear();
...
void Assembly::applyTractionBoundary(int g_iteration) {
  ...
  for (PeriParticlePArray::iterator peri_pt = topBoundaryInnerVec.begin();
       peri_pt != topBoundaryInnerVec.end(); peri_pt++) {
    (*peri_pt)->addAccelerationByForce(tmp_vec);
  }
  for (PeriParticlePArray::iterator peri_pt = topBoundaryEdgeVec.begin();
       peri_pt != topBoundaryEdgeVec.end(); peri_pt++) {
    (*peri_pt)->addAccelerationByForce(tmp_vec * 0.5);
  }
  for (PeriParticlePArray::iterator peri_pt = topBoundaryCornerVec.begin();
       peri_pt != topBoundaryCornerVec.end(); peri_pt++) {
    (*peri_pt)->addAccelerationByForce(tmp_vec * 0.25);
  }
  for (PeriParticlePArray::iterator peri_pt = bottomBoundaryVec.begin();
       peri_pt != bottomBoundaryVec.end(); peri_pt++) {
    (*peri_pt)->addAccelerationByForce(-tmp_vec);
  }
...
void Assembly::rigidInclusion() {
  ...
  if (mpiRank == 0) {
    ...
    REAL z2 = dem::Parameter::getSingleton().parameter["Zmax"];
    setContainer(Rectangle(x1, y1, z1, x2, y2, z2));
    setGrid(Rectangle(x1, y1, z1, x2, y2, z2));  // compute grid assumed to be
    ...
    calcParticleVolume();  // volume and horizon size are related to the mesh,
                           // July 14, 2014
    calcHorizonSize();  // so these two should be calculated before peri-points
  ...
  // printRecvPeriDomain("peridomain_recv_rank_0.dat");    // bondVec is not
  // empty
  //}
  // if(mpiRank==1){
  ...
  char cstr0[50];
  if (mpiRank == 0) {
    plotGrid(strcat(combineString(cstr0, "rigidInc_gridplot_", iterSnap - 1, 3),
                    ".dat"));
  ...
  commuParticle();            // the commuPeriParticle() have to be called after
                              // commuParticle(), since
  commuPeriParticle();        // rankX1... are calculated in commuParticle()
  constructRecvPeriMatrix();  // construct the Matrix members in
                              // recvPeriParticle
  ...
  pt!= topBoundaryInnerVec.end(); pt++) {
          ...
          ofs_top << std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  pt!= topBoundaryEdgeVec.end(); pt++) {
          ...
          ofs_bot << std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  pt!= topBoundaryCornerVec.end(); pt++) {
          ...
          ofs_fix << std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  pt!= bottomBoundaryVec.end(); pt++) {
          ...
          ofs2<< std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  leftBoundaryVec.end(); pt++) {
          ...
          ofs3<< std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  frontBoundaryVec.end(); pt++) {
          ...
          ofs4<< std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  pt!= fixedPeriParticleVec.end(); pt++) {
          ...
          ofs5<< std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  periParticleVec.end(); pt++) {
          ...
          ofs6 << std::setw(20) << (*pt)->getInitPosition().getX() +
  (*pt)->getDisplacement().getX()
          << std::setw(20) << (*pt)->getInitPosition().getY() +
  (*pt)->getDisplacement().getY()
          << std::setw(20) << (*pt)->getInitPosition().getZ() +
  ...
  */
  while (iteration <= endStep) {
    ...
    calcParticleStress();
    calcParticleAcceleration();  // peri-point acceleration is set zero at the
                                 // beginning
    ...
    if (iteration % (netStep / netSnap) == 0) {
      ...
      if (mpiRank == 0) {
        plotBoundary(strcat(
            combineString(cstr, "rigidInc_bdryplot_", iterSnap, 3), ".dat"));
        plotGrid(strcat(combineString(cstr, "rigidInc_gridplot_", iterSnap, 3),
                        ".dat"));
        printParticle(combineString(cstr, "rigidInc_particle_", iterSnap, 3));
        printBdryContact(
            combineString(cstr, "rigidInc_bdrycntc_", iterSnap, 3));
    ...
    releaseRecvParticle();      // late release because printContact refers to
                                // received particles
    releaseRecvPeriParticle();  // release recvPeriParticleVec and also remove
    ...
    //      if (mpiRank == 0 && (iteration+1 ) % (netStep / netSnap) == 0) //
