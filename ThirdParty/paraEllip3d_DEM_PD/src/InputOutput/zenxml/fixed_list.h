...
namespace zen {
// std::list(C++11)-like class for inplace element construction supporting
...
public:
  ...
  {
  public:
    FixedIterator(NodeT* it = nullptr)
      : it_(it)
    {
    }
    ...
    inline friend bool operator==(const FixedIterator& lhs,
                                  const FixedIterator& rhs)
    {
...
private:
  ...
  Node* lastInsert_ =
    nullptr; // point to last insertion; required by efficient emplace_back()
  size_t sz_ = 0;
};

// just as fast as FixedList, but simpler, more CPU-cache-friendly => superseeds
// FixedList!
template <class T>
class FixedVector
{
public:
  ...
  /*
  class EndIterator {}; //just like FixedList: no iterator invalidation after
  emplace_back()

  template <class V>
  class FixedIterator : public std::iterator<std::forward_iterator_tag, V>
  //could make this random-access if needed
  {
  public:
      FixedIterator(std::vector<std::unique_ptr<T>>& cont, size_t pos) :
  cont_(cont), pos_(pos) {}
      FixedIterator& operator++() { ++pos_; return *this; }
      inline friend bool operator==(const FixedIterator& lhs, EndIterator) {
  return lhs.pos_ == lhs.cont_.size(); }
      inline friend bool operator!=(const FixedIterator& lhs, EndIterator) {
  ...
  */
  ...
  class FixedIterator
    : public std::iterator<std::forward_iterator_tag,
                           V> // could make this bidirectional if needed
  {
  public:
    FixedIterator(IterImpl it)
      : it_(it)
    {
    }
    FixedIterator& operator++()
    ...
    V* operator->() const { return &**it_; }
  private:
    IterImpl it_; // TODO: avoid iterator invalidation after emplace_back();
                  // caveat: end() must not store old length!
  };

  using value_type = T;
  using iterator =
    FixedIterator<typename std::vector<std::unique_ptr<T>>::iterator, T>;
  using const_iterator =
    FixedIterator<typename std::vector<std::unique_ptr<T>>::const_iterator,
                  const T>;
  using reference = T&;
  using const_reference = const T&;
