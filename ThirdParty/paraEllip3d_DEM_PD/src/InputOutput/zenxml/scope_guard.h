...
// partially specialize scope guard destructor code and get rid of those pesky
// MSVC "4127 conditional expression is constant"
template <typename F>
inline void
runScopeGuardDestructor(
  F& fun, int /*exeptionCountOld*/,
  StaticEnum<ScopeGuardRunMode, ScopeGuardRunMode::ON_EXIT>)
{
  try {
    fun();
  } catch (...) {
    assert(false);
  } // consistency: don't expect exceptions for ON_EXIT even if "!failed"!
}
...
runScopeGuardDestructor(
  F& fun, int exeptionCountOld,
  StaticEnum<ScopeGuardRunMode, ScopeGuardRunMode::ON_SUCCESS>)
{
...
runScopeGuardDestructor(
  F& fun, int exeptionCountOld,
  StaticEnum<ScopeGuardRunMode, ScopeGuardRunMode::ON_FAIL>)
{
  const bool failed = getUncaughtExceptionCount() > exeptionCountOld;
  if (failed)
    try {
...
{
public:
  explicit ScopeGuard(const F& fun)
    : fun_(fun)
  {
  }
  explicit ScopeGuard(F&& fun)
    : fun_(std::move(fun))
  {
  }
  ...
  {
    if (!dismissed_)
      runScopeGuardDestructor(fun_, exeptionCount_,
                              StaticEnum<ScopeGuardRunMode, runMode>());
  }
...
template <ScopeGuardRunMode runMode, class F>
inline auto
makeGuard(F&& fun)
{
  return ScopeGuard<runMode, std::decay_t<F>>(std::forward<F>(fun));
}
}
...
#define ZEN_ON_SCOPE_EXIT(X)                                                   \
  auto ZEN_CONCAT(dummy, __LINE__) =                                           \
    zen::makeGuard<zen::ScopeGuardRunMode::ON_EXIT>([&] { X; });               \
  (void)ZEN_CONCAT(dummy, __LINE__);
#define ZEN_ON_SCOPE_FAIL(X)                                                   \
  auto ZEN_CONCAT(dummy, __LINE__) =                                           \
    zen::makeGuard<zen::ScopeGuardRunMode::ON_FAIL>([&] { X; });               \
  (void)ZEN_CONCAT(dummy, __LINE__);
#define ZEN_ON_SCOPE_SUCCESS(X)                                                \
  auto ZEN_CONCAT(dummy, __LINE__) =                                           \
    zen::makeGuard<zen::ScopeGuardRunMode::ON_SUCCESS>([&] { X; });            \
  (void)ZEN_CONCAT(dummy, __LINE__);
