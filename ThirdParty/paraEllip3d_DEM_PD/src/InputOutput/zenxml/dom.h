...
public:
  ...
  explicit XmlElement(const String& name, XmlElement* parentElement = nullptr)
    : name_(utfCvrtTo<std::string>(name))
  ...
  */
  ...
  /**
    \tparam T Arbitrary user data type: e.g. any string class, all built-in
    arithmetic numbers, STL container, ...
    \returns "true" if Xml element was successfully converted to value, cannot
    fail for string-like types
  */
  template <class T>
  bool getValue(T& value) const
  {
    return readStruc(*this, value);
  }

  /// Set the value of this element.
  /**
    \tparam T Arbitrary user data type: e.g. any string-like type, all built-in
    arithmetic numbers, STL container, ...
  */
  template <class T>
  void setValue(const T& value)
  {
    writeStruc(value, *this);
  }

  /// Retrieve an attribute by name.
  /**
    \tparam String Arbitrary string-like type: e.g. std::string, wchar_t*,
    char[], wchar_t, wxString, MyStringClass, ...
    \tparam T String-convertible user data type: e.g. any string class, all
  ...
  */
  ...
  {
    attributes.erase(utfCvrtTo<std::string>(name));
  }
  ...
  */
  ...
  XmlElement* getChild(const String& name)
  {
    return const_cast<XmlElement*>(
      static_cast<const XmlElement*>(this)->getChild(name));
  }
  ...
  class PtrIter : public std::iterator<std::input_iterator_tag, T>,
                  private AccessPolicy // get rid of shared_ptr indirection
  {
  public:
    PtrIter(IterTy it)
      : it_(it)
    ...
    inline friend bool operator==(const PtrIter& lhs, const PtrIter& rhs)
    {
  ...
  struct AccessMapElement
  {
    ...
    T& objectRef(const IterTy& it)
    {
      return *(it->second);
    }
  };

  using ChildIter2 = PtrIter<std::multimap<std::string, XmlElement*>::iterator,
                             XmlElement, AccessMapElement>;
  using ChildIterConst2 =
    PtrIter<std::multimap<std::string, XmlElement*>::const_iterator,
            const XmlElement, AccessMapElement>;
  ...
  /**
    ...
    \return A pair of STL begin/end iterators to access the child elements
    sequentially.
  */
  template <class String>
  std::pair<ChildIterConst2, ChildIterConst2> getChildren(
  ...
  using ChildIter =
    PtrIter<FixedList<XmlElement>::iterator, XmlElement, AccessListElement>;
  using ChildIterConst = PtrIter<FixedList<XmlElement>::const_iterator,
                                 const XmlElement, AccessListElement>;
  ...
  /**
    ...
    std::for_each(iterPair.first, iterPair.second,
          [](const XmlElement& child) { ... });
    \endcode
    \return A pair of STL begin/end iterators to access all child elements
    sequentially.
  */
  ...
  /* -> disabled documentation extraction
    ...
    \endcode
    \return A pair of STL begin/end iterators to access all attributes
    sequentially as a list of name/value pairs of std::string.
  */
  std::pair<AttrIter, AttrIter> getAttributes() const
  {
    return std::make_pair(attributes.begin(), attributes.end());
  }

  // swap two elements while keeping references to parent.  -> disabled
  ...
  void swapSubtree(XmlElement& other)
  {
    ...
    // std::swap(parent_, other.parent_); -> parent is physical location; update
    // children's parent reference instead:
...
private:
  ...
  FixedList<XmlElement> childElements; // all child elements in order of
                                       // creation
  std::multimap<std::string, XmlElement*>
    childElementsSorted; // alternate key: sorted by element name
  XmlElement* parent_;
};

// XmlElement::setValue<T>() calls zen::writeStruc() which calls
// XmlElement::setValue() ... => these two specializations end the circle
template <>
inline void
...
template <>
inline bool
XmlElement::getValue(std::string& value) const
{
  value = value_;
  return true;
}
...
{
public:
  /// Default constructor setting up an empty XML document with a standard
  ...
  */
  ...
  {
    return utfCvrtTo<String>(version_);
  }
  ...
  */
  ...
  {
    version_ = utfCvrtTo<std::string>(version);
  }
  ...
  */
  ...
  {
    return utfCvrtTo<String>(encoding_);
  }
  ...
  */
  ...
  {
    encoding_ = utfCvrtTo<std::string>(encoding);
  }
  ...
  */
  ...
  {
    return utfCvrtTo<String>(standalone_);
  }
  ...
  */
  ...
  {
    standalone_ = utfCvrtTo<std::string>(standalone);
  }
...
private:
  XmlDoc(const XmlDoc&) =
    delete; // not implemented, thanks to XmlElement::parent_
