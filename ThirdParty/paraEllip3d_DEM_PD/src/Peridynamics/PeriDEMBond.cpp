...
void PeriDEMBond::applyBondForce() {
  ...
  Vec currProjectorGlobal = demParticle->localToGlobal(initProjectorLocal) +
                            demParticle->getCurrPos();
...
a lot of computations,
	...
	// (1) calculate current bond vector
	// get current global coordinates of the initProjectorLocal
	REAL currBondL = vfabs(periPoint->getCurrPosition()-demParticle->getCurrPos())
- demParticle->getA();
...
dem::Parameter::getSingleton().parameter["periYoung"]*1e-1;	// just in value
	Vec fn = (currBondL-initBondL)*kn_periBndry*normalize(initBondVec);	// force is
...
void PeriDEMBond::applyBondBoundary() {
  ...
  // bond created
  dem::Vec curr_posi_global = demParticle->localToGlobal(initProjectorLocal) +
                              demParticle->getCurrPos();
