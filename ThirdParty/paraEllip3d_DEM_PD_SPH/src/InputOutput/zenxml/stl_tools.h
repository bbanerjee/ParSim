...
template <class KeyType, class ValueType, class LessType, class Alloc,
          class Predicate>
...
V& map_add_or_update(M& map, const K& key,
                     const V& value); // efficient add or update without
...
ForwardIterator binary_search(ForwardIterator first, ForwardIterator last,
                              const T& value, CompLess less);

template <class BidirectionalIterator, class T>
BidirectionalIterator find_last(BidirectionalIterator first,
                                BidirectionalIterator last, const T& value);

// replacement for std::find_end taking advantage of bidirectional iterators
// (and giving the algorithm a reasonable name)
template <class BidirectionalIterator1, class BidirectionalIterator2>
BidirectionalIterator1 search_last(BidirectionalIterator1 first1,
...
template <class T, class Alloc, class Predicate>
inline void
erase_if(std::vector<T, Alloc>& v, Predicate p)
{
...
inline void
set_or_map_erase_if(S& s, Predicate p)
{
...
template <class T, class LessType, class Alloc, class Predicate>
inline void
erase_if(std::set<T, LessType, Alloc>& s, Predicate p)
{
  impl::set_or_map_erase_if(s, p);
} // don't make this any more generic! e.g. must not compile for std::vector!!!

template <class KeyType, class ValueType, class LessType, class Alloc,
          class Predicate>
inline void
erase_if(std::map<KeyType, ValueType, LessType, Alloc>& m, Predicate p)
{
  impl::set_or_map_erase_if(m, p);
}

template <class T, class Alloc, class C>
inline void
append(std::vector<T, Alloc>& v, const C& c)
{
...
template <class T, class LessType, class Alloc, class C>
inline void
append(std::set<T, LessType, Alloc>& s, const C& c)
{
...
template <class KeyType, class ValueType, class LessType, class Alloc, class C>
inline void
append(std::map<KeyType, ValueType, LessType, Alloc>& m, const C& c)
{
  m.insert(c.begin(), c.end());
}

template <class M, class K, class V>
inline V& map_add_or_update(M& map, const K& key,
                            const V& value) // efficient add or update without
                                            // "default-constructible"
                                            // requirement (Effective STL, item
                                            // 24)
{
...
template <class T, class Alloc>
inline void
removeDuplicates(std::vector<T, Alloc>& v)
{
...
find_last(const BidirectionalIterator first, const BidirectionalIterator last,
          const T& value)
{
  for (BidirectionalIterator it = last;
       it != first;) // reverse iteration: 1. check 2. decrement 3. evaluate
  {
    --it; //
...
search_last(const BidirectionalIterator1 first1, BidirectionalIterator1 last1,
            const BidirectionalIterator2 first2,
            const BidirectionalIterator2 last2)
{
...
static_assert(false, "VS2010 performance bug in std::unordered_set<>: "
                     "http://drdobbs.com/blogs/cpp/232200410 -> should be "
                     "fixed in VS11");
#endif
...
template <class ByteIterator>
inline size_t
hashBytesAppend(size_t hashVal, ByteIterator first, ByteIterator last)
{
#ifdef ZEN_BUILD_32BIT
