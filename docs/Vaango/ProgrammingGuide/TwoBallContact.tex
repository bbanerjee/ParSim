...
\begin{document}
  ...
  \begingroup
    ...
    \textcolor{white}{Understanding the Two Ball Contact Program}\par % Book title
    \vspace*{0.5cm}
    {\Large \textcolor{white}{written in Python using MPM}}\par
    \vspace*{1cm}
...
\end{lstlisting}
\emph{simplecontact} class is where you can check when the contact between your objects has occurred. (--> \textbf{Chapter}~\ref{chap:simplecontact})\\ \\
...
\end{lstlisting}
\item Here check if the contact has occurred, the internal force for the particles of each object should be added together. The new internal force for both of the particles of the objects is the same and equals to the summation of the \TT{gfi} from each (\textbf{Chapter}~\ref{chap:simplecontact}).  --> \TT{exchForceInterpolated}
\begin{lstlisting}tail
