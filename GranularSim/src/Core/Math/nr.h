...
void amebsa(REAL** p, REAL y[], int ndim, REAL pb[], REAL* yb, REAL ftol,
            REAL (*funk)(REAL[]), int* iter, REAL temptr);
void amoeba(REAL** p, REAL y[], int ndim, REAL ftol, REAL (*funk)(REAL[]),
            int* iter);
REAL amotry(REAL** p, REAL y[], REAL psum[], int ndim, REAL (*funk)(REAL[]),
            int ihi, REAL fac);
REAL amotsa(REAL** p, REAL y[], REAL psum[], int ndim, REAL pb[], REAL* yb,
            REAL (*funk)(REAL[]), int ihi, REAL* yhi, REAL fac);
...
void bcucof(REAL y[], REAL y1[], REAL y2[], REAL y12[], REAL d1, REAL d2,
            REAL** c);
void bcuint(REAL y[], REAL y1[], REAL y2[], REAL y12[], REAL x1l, REAL x1u,
            REAL x2l, REAL x2u, REAL x1, REAL x2, REAL* ansy, REAL* ansy1,
            REAL* ansy2);
void beschb(REAL x, REAL* gam1, REAL* gam2, REAL* gampl, REAL* gammi);
...
REAL bessj0(REAL x);
REAL bessj1(REAL x);
void bessjy(REAL x, REAL xnu, REAL* rj, REAL* ry, REAL* rjp, REAL* ryp);
...
void broydn(REAL x[], int n, int* check, void (*vecfunc)(int, REAL[], REAL[]));
...
void chsone(REAL bins[], REAL ebins[], int nbins, int knstrn, REAL* df,
            REAL* chsq, REAL* prob);
void chstwo(REAL bins1[], REAL bins2[], int nbins, int knstrn, REAL* df,
            REAL* chsq, REAL* prob);
void cisi(REAL x, REAL* ci, REAL* si);
void cntab1(int** nn, int ni, int nj, REAL* chisq, REAL* df, REAL* prob,
            REAL* cramrv, REAL* ccc);
void cntab2(int** nn, int ni, int nj, REAL* h, REAL* hx, REAL* hy, REAL* hygx,
            REAL* hxgy, REAL* uygx, REAL* uxgy, REAL* uxy);
...
void cyclic(REAL a[], REAL b[], REAL c[], REAL alpha, REAL beta, REAL r[],
            REAL x[], unsigned long n);
void daub4(REAL a[], unsigned long n, int isign);
REAL dawson(REAL x);
REAL dbrent(REAL ax, REAL bx, REAL cx, REAL (*f)(REAL), REAL (*df)(REAL),
            REAL tol, REAL* xmin);
...
void dftcor(REAL w, REAL delta, REAL a, REAL b, REAL endpts[], REAL* corre,
            REAL* corim, REAL* corfac);
void dftint(REAL (*func)(REAL), REAL a, REAL b, REAL w, REAL* cosint,
            REAL* sinint);
void difeq(int k, int k1, int k2, int jsf, int is1, int isf, int indexv[],
           int ne, REAL** s, REAL** y);
void dlinmin(REAL p[], REAL xi[], int n, REAL* fret, REAL (*func)(REAL[]),
             void (*dfunc)(REAL[], REAL[]));
...
void dsprstx(REAL sa[], unsigned long ija[], REAL x[], REAL b[],
             unsigned long n);
void dsvbksb(REAL** u, REAL w[], REAL** v, int m, int n, REAL b[], REAL x[]);
...
void fit(REAL x[], REAL y[], int ndata, REAL sig[], int mwt, REAL* a, REAL* b,
         REAL* siga, REAL* sigb, REAL* chi2, REAL* q);
void fitexy(REAL x[], REAL y[], int ndat, REAL sigx[], REAL sigy[], REAL* a,
...
void fred2(int n, REAL a, REAL b, REAL t[], REAL f[], REAL w[], REAL (*g)(REAL),
           REAL (*ak)(REAL, REAL));
...
REAL golden(REAL ax, REAL bx, REAL cx, REAL (*f)(REAL), REAL tol, REAL* xmin);
...
void ks2d1s(REAL x1[], REAL y1[], unsigned long n1,
            void (*quadvl)(REAL, REAL, REAL*, REAL*, REAL*, REAL*), REAL* d1,
            REAL* prob);
...
void lfit(REAL x[], REAL y[], REAL sig[], int ndat, REAL a[], int ia[], int ma,
          REAL** covar, REAL* chisq, void (*funcs)(REAL, REAL[], int));
void linbcg(unsigned long n, REAL b[], REAL x[], int itol, REAL tol, int itmax,
            int* iter, REAL* err);
...
void machar(int* ibeta, int* it, int* irnd, int* ngrd, int* machep, int* negep,
            int* iexp, int* minexp, int* maxexp, REAL* eps, REAL* epsneg,
            REAL* xmin, REAL* xmax);
...
void mppi(int n);
void mprove(REAL** a, REAL** alud, int n, int indx[], REAL b[], REAL x[]);
...
void mpsub(int* is, unsigned char w[], unsigned char u[], unsigned char v[],
           int n);
void mrqcof(REAL x[], REAL y[], REAL sig[], int ndata, REAL a[], int ia[],
            int ma, REAL** alpha, REAL beta[], REAL* chisq,
            void (*funcs)(REAL, REAL[], REAL*, REAL[], int));
void mrqmin(REAL x[], REAL y[], REAL sig[], int ndata, REAL a[], int ia[],
            int ma, REAL** covar, REAL** alpha, REAL* chisq,
            void (*funcs)(REAL, REAL[], REAL*, REAL[], int), REAL* alamda);
void newt(REAL x[], int n, int* check, void (*vecfunc)(int, REAL[], REAL[]));
void odeint(REAL ystart[], int nvar, REAL x1, REAL x2, REAL eps, REAL h1,
            REAL hmin, int* nok, int* nbad,
            void (*derivs)(REAL, REAL[], REAL[]),
            void (*rkqs)(REAL[], REAL[], int, REAL*, REAL, REAL, REAL[], REAL*,
                         REAL*, void (*)(REAL, REAL[], REAL[])));
void orthog(int n, REAL anu[], REAL alpha[], REAL beta[], REAL a[], REAL b[]);
void pade(REAL cof[], int n, REAL* resid);
void pccheb(REAL d[], REAL c[], int n);
void pcshft(REAL a, REAL b, REAL d[], int n);
void pearsn(REAL x[], REAL y[], unsigned long n, REAL* r, REAL* prob, REAL* z);
void period(REAL x[], REAL y[], int n, REAL ofac, REAL hifac, REAL px[],
            REAL py[], int np, int* nout, int* jmax, REAL* prob);
void piksr2(int n, REAL arr[], REAL brr[]);
void piksrt(int n, REAL arr[]);
void pinvs(int ie1, int ie2, int je1, int jsf, int jc1, int k, REAL*** c,
           REAL** s);
...
void polin2(REAL x1a[], REAL x2a[], REAL** ya, int m, int n, REAL x1, REAL x2,
            REAL* y, REAL* dy);
...
void quadct(REAL x, REAL y, REAL xx[], REAL yy[], unsigned long nn, REAL* fa,
            REAL* fb, REAL* fc, REAL* fd);
...
void ratlsq(REAL (*fn)(REAL), REAL a, REAL b, int mm, int kk, REAL cof[],
            REAL* dev);
...
void red(int iz1, int iz2, int jz1, int jz2, int jm1, int jm2, int jmf, int ic1,
         int jc1, int jcf, int kc, REAL*** c, REAL** s);
...
REAL rtnewt(void (*funcd)(REAL, REAL*, REAL*), REAL x1, REAL x2, REAL xacc);
REAL rtsafe(void (*funcd)(REAL, REAL*, REAL*), REAL x1, REAL x2, REAL xacc);
...
void simp1(REAL** a, int mm, int ll[], int nll, int iabf, int* kp, REAL* bmax);
...
void simplx(REAL** a, int m, int n, int m1, int m2, int m3, int* icase,
            int izrov[], int iposv[]);
void simpr(REAL y[], REAL dydx[], REAL dfdx[], REAL** dfdy, int n, REAL xs,
...
void solvde(int itmax, REAL conv, REAL slowc, REAL scalv[], int indexv[],
            int ne, int nb, int m, REAL** y, REAL*** c, REAL** s);
void sor(REAL** a, REAL** b, REAL** c, REAL** d, REAL** e, REAL** f, REAL** u,
         int jmax, REAL rjac);
...
void sprstx(REAL sa[], unsigned long ija[], REAL x[], REAL b[],
            unsigned long n);
void stifbs(REAL y[], REAL dydx[], int nv, REAL* xx, REAL htry, REAL eps,
            REAL yscal[], REAL* hdid, REAL* hnext,
            void (*derivs)(REAL, REAL[], REAL[]));
void stiff(REAL y[], REAL dydx[], int n, REAL* x, REAL htry, REAL eps,
           REAL yscal[], REAL* hdid, REAL* hnext,
           void (*derivs)(REAL, REAL[], REAL[]));
...
void svdfit(REAL x[], REAL y[], REAL sig[], int ndata, REAL a[], int ma,
            REAL** u, REAL** v, REAL w[], REAL* chisq,
...
void tridag(REAL a[], REAL b[], REAL c[], REAL r[], REAL u[], unsigned long n);
...
void wtn(REAL a[], unsigned long nn[], int ndim, int isign,
         void (*wtstep)(REAL[], unsigned long, int));
void wwghts(REAL wghts[], int n, REAL h, void (*kermom)(REAL[], REAL, int));
int zbrac(REAL (*func)(REAL), REAL* x1, REAL* x2);
void zbrak(REAL (*fx)(REAL), REAL x1, REAL x2, int n, REAL xb1[], REAL xb2[],
           int* nb);
