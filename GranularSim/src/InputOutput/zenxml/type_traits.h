...
namespace zen {
//################# TMP compile time return values: "inherit to return
// compile-time result" ##############
template <int i>
struct StaticInt
{
...
// Herb Sutter's signedness conversion helpers:
...
template <class T>
struct IsInteger; // IsSignedInt or IsUnsignedInt
template <class T>
struct IsArithmetic; // IsInteger or IsFloat
...
/*  Detect data or function members of a class by name: ZEN_INIT_DETECT_MEMBER +
   HasMember_
    Example: 1. ZEN_INIT_DETECT_MEMBER(c_str);
             2. HasMember_c_str<T>::value     -> use boolean
*/

/*  Detect data or function members of a class by name *and* type:
   ZEN_INIT_DETECT_MEMBER2 + HasMember_
...
//################ implementation ######################
#define ZEN_SPECIALIZE_TRAIT(X, Y)                                             \
  template <>                                                                  \
  struct X<Y> : TrueType                                                       \
  {                                                                            \
  };
...
ZEN_SPECIALIZE_TRAIT(IsUnsignedInt, unsigned long long int); // new with C++11 -
                                                             // same type as
                                                             // unsigned __int64
                                                             // in VS2010
//------------------------------------------------------
...
ZEN_SPECIALIZE_TRAIT(
  IsSignedInt,
  long long int); // new with C++11 - same type as __int64 in VS2010
//------------------------------------------------------
...
#define ZEN_INIT_DETECT_MEMBER(NAME)                                           \
  ...
  private:                                                                     \
    ...
    {                                                                          \
    }; /*this works only for class types!!!*/                                  \
                                                                               \
    template <class U>                                                         \
    static No& hasMember(Helper<int Fallback::*, &Helper2<U>::NAME>*);         \
    template <class U>                                                         \
    static Yes& hasMember(...);                                                \
                                                                               \
  public:                                                                      \
    enum                                                                       \
    {                                                                          \
      value = sizeof(hasMember<T>(nullptr)) == sizeof(Yes)                     \
    };                                                                         \
  };                                                                           \
                                                                               \
  template <class T>                                                           \
  ...
  struct HasMember_##NAME                                                      \
    : StaticBool<HasMemberImpl_##NAME<std::is_class<T>::value, T>::value>      \
  {                                                                            \
  };
...
#define ZEN_INIT_DETECT_MEMBER2(NAME, TYPE)                                    \
  ...
  {                                                                            \
    ...
    {                                                                          \
                                                                               ...
                                                                               \
    template <class T>                                                         \
    static Yes& hasMember(Helper<TYPE, &T::NAME>*);                            \
    template <class T>                                                         \
    static No& hasMember(...);                                                 \
                                                                               \
  public:                                                                      \
    enum                                                                       \
    {                                                                          \
      value = sizeof(hasMember<U>(nullptr)) == sizeof(Yes)                     \
    };                                                                         \
  };
//####################################################################
...
#define ZEN_INIT_DETECT_MEMBER_TYPE(TYPENAME)                                  \
  ...
  {                                                                            \
    ...
    {                                                                          \
                                                                               ...
                                                                               \
    template <class U>                                                         \
    static Yes& hasMemberType(Helper<typename U::TYPENAME>*);                  \
    template <class U>                                                         \
    static No& hasMemberType(...);                                             \
                                                                               \
  public:                                                                      \
    enum                                                                       \
    {                                                                          \
      value = sizeof(hasMemberType<T>(nullptr)) == sizeof(Yes)                 \
    };                                                                         \
  };
}
