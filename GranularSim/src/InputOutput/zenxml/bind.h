...
/**
Convenience function that does nothing more than calling loadStream() and
parse().
...
inline XmlDoc load(
  const String& filename) // throw XmlFileError, XmlParsingError
{
...
/**
Convenience function that does nothing more than calling serialize() and
saveStream().
...
*/
template <class String>
inline void save(const XmlDoc& doc, const String& filename,
...
public:
  ...
  */
  ...
  /**
  This conversion requires a specialization of zen::writeText() or
  zen::writeStruc() for type T.
  \tparam T User type that is converted into an XML element value.
  */
  template <class T>
  void operator()(const T& value)
  {
    writeStruc(value, *ref_);
  }
  ...
  \tparam String Arbitrary string-like type: e.g. std::string, wchar_t*, char[],
  wchar_t, wxString, MyStringClass, ...
  \tparam T String-convertible user data type: e.g. any string-like type, all
  built-in arithmetic numbers
  \sa XmlElement::setAttribute()
  */
  template <class String, class T>
  void attribute(const String& name, const T& value)
  {
    ref_->setAttribute(name, value);
  }
...
public:
  ...
  /**
    \code
      ...
      zen::XmlIn in(doc);
      in["elem1"](value1); //
      in["elem2"](value2); //read data from XML elements into variables
    "value1", "value2", "value3"
      in["elem3"](value3); //
    \endcode
  */
  XmlIn(const XmlDoc& doc)
    : log(std::make_shared<ErrorLog>())
  {
    refList.push_back(&doc.root());
  }
  ...
  */
  ...
  /**
  It is \b not an error if the child element does not exist, but only later if a
  conversion to user data is attempted.
  \tparam String Arbitrary string-like type: e.g. std::string, wchar_t*, char[],
  wchar_t, wxString, MyStringClass, ...
  \param name The name of the child element
  */
  ...
  XmlIn operator[](const String& name) const
  {
    ...
    return XmlIn(childList, childList.empty() ? getChildNameFormatted(name)
                                              : std::string(),
                 log);
  }
  ...
  */
  ...
  /**
  This conversion requires a specialization of zen::readText() or
  zen::readStruc() for type T.
  ...
  */
  ...
  \code
      ...
      in["elem"].attribute("attr2", value2); //read data from XML attributes
  into variables "value1", "value2", "value3"
  ...
  \tparam T String-convertible user data type: e.g. any string-like type, all
  built-in arithmetic numbers
  \returns "true" if the attribute was found and the conversion to the output
  ...
  */
  ...
  bool attribute(const String& name, T& value) const
  {
    if (refIndex < refList.size()) {
      ...
      if (!success)
        log->notifyMissingAttribute(getNameFormatted(),
                                    utfCvrtTo<std::string>(name));
      return success;
    } else {
      log->notifyMissingElement(getNameFormatted());
      return false;
  ...
  {
    return refIndex < refList.size() ? refList[refIndex] : nullptr;
  }
  ...
  /**
    \code
       ...
       if (child)
         ...
    \endcode
    Use member pointer as implicit conversion to bool (C++ Templates -
    Vandevoorde/Josuttis; chapter 20)
  */
  explicit operator bool() const { return get() != nullptr; }

  /// Notifies errors while mapping the XML to user data
  /**
  Error logging is shared by each hiearchy of XmlIn proxy instances that are
  created from each other. Consequently it doesn't matter which instance you
  query for errors:
    \code
      XmlIn in(doc);
      ...
      assert(in.getErrorsAs<std::string>() ==
  inItem.getErrorsAs<std::string>());
    \endcode

    Note that error logging is \b NOT global, but owned by all instances of a
  hierarchy of XmlIn proxies.
    Therefore it's safe to use unrelated XmlIn proxies in multiple threads.
    \n\n
    However be aware that the chain of connected proxy instances will be broken
  once you call XmlIn::get() to retrieve the underlying pointer.
    Errors that occur when working with this pointer are not logged by the
  original set of related instances.
  */
  bool errorsOccured() const { return !log->elementList().empty(); }

  /// Get a list of XML element and attribute names which failed to convert to
  /// user data.
  /**
    \tparam String Arbitrary string class: e.g. std::string, std::wstring,
    wxString, MyStringClass, ...
    \returns A list of XML element and attribute names, empty list if no errors
    occured.
  */
  template <class String>
  std::vector<String> getErrorsAs() const
  {
    ...
    std::transform(
      elements.begin(), elements.end(), std::back_inserter(output),
      [](const std::string& str) { return utfCvrtTo<String>(str); });
    return output;
  }
...
private:
  XmlIn(const std::vector<const XmlElement*>& siblingList,
    ...
    : refList(siblingList)
    , formattedName(elementNameFmt)
    , log(sharedlog)
  {
    assert((!siblingList.empty() && elementNameFmt.empty()) ||
           (siblingList.empty() && !elementNameFmt.empty()));
  }

  static std::string getNameFormatted(
    const XmlElement& elem) //"<Root> <Level1> <Level2>"
  {
    return (elem.parent() ? getNameFormatted(*elem.parent()) + " "
                          : std::string()) +
  ...
  std::string getChildNameFormatted(const std::string& childName) const
  {
    ...
    return (parentName.empty() ? std::string() : (parentName + " ")) + "<" +
           childName + ">";
  }
  ...
  {
  public:
    void notifyConversionError(const std::string& displayName)
    {
      insert(displayName);
    }
    void notifyMissingElement(const std::string& displayName)
    {
      insert(displayName);
    }
    void notifyMissingAttribute(const std::string& displayName,
                                const std::string& attribName)
    {
      insert(displayName + " @" + attribName);
    }

    const std::vector<std::string>& elementList() const
    {
  ...
  std::vector<const XmlElement*>
    refList; // all sibling elements with same name (all pointers bound!)
  size_t refIndex = 0;       // this sibling's index in refList
  std::string formattedName; // contains full and formatted element name if (and
                             // only if) refList is empty
