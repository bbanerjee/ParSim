...
/**
\file
\brief Convert an XML document object model (class XmlDoc) to and from a byte
...
{
  XmlParsingError(size_t rowNo, size_t colNo)
    : row(rowNo)
    , col(colNo)
  {
  }
...
inline std::string normalize(const std::string& str,
                             Predicate pred) // pred: unary function taking a
                                             // char, return true if value shall
                                             // be encoded as hex
{
  std::string output;
  for (const char c : str) {
    if (c == '&') //
      output += "&amp;";
             ...
             '<') // normalization mandatory: http://www.w3.org/TR/xml/#syntax
      output += "&lt;";
    else if (c == '>') //
      output += "&gt;";
    else if (pred(c)) {
...
inline std::string
normalizeName(const std::string& str)
{
  return normalize(str, [](char c) {
    return isWhiteSpace(c) || c == '=' || c == '/' || c == '\'' || c == '\"';
  });
}

inline std::string
normalizeElementValue(const std::string& str)
{
...
inline std::string
normalizeAttribValue(const std::string& str)
{
  return normalize(str, [](char c) {
...
template <class CharIterator, size_t N>
inline bool
checkEntity(CharIterator& first, CharIterator last,
            const char (&placeholder)[N])
{
  assert(placeholder[N - 1] == 0);
  const ptrdiff_t strLen = N - 1; // don't count null-terminator
  if (last - first >= strLen &&
      std::equal(first, first + strLen, placeholder)) {
    first += strLen - 1;
    return true;
  }
  return false;
...
namespace {
std::string
denormalize(const std::string& str)
{
  std::string output;
  for (auto it = str.begin(); it != str.end(); ++it) {
    ...
    if (c == '&') {
      if (checkEntity(it, str.end(), "&amp;"))
        ...
        output += '\"';
      else if (str.end() - it >= 6 && it[1] == '#' && it[2] == 'x' &&
               it[5] == ';') {
          ...
          size_t indentLevel)
{
  ...
  if (iterPair.first != iterPair.second) // structured element
  {
    ...
    std::for_each(iterPair.first, iterPair.second, [&](const XmlElement& el) {
    ...
    if (!value.empty()) // value element
      stream +=
        '>' + normalizeElementValue(value) + "</" + nameFmt + '>' + lineBreak;
    else // empty element
      stream += "/>" + lineBreak;
  }
}
          ...
          const std::string& indent)
{
  ...
  std::string output =
    "<?xml" + version + encoding + standalone + "?>" + lineBreak;
...
public:
  ...
  {
    // skip whitespace
    pos = std::find_if(pos, stream_.end(),
                       [](char c) { return !zen::isWhiteSpace(c); });
    ...
    if (startsWith(xmlCommentBegin)) {
      auto it = std::search(pos + xmlCommentBegin.size(), stream_.end(),
                            xmlCommentEnd.begin(), xmlCommentEnd.end());
      if (it != stream_.end()) {
  ...
  {
    auto it = std::find_if(pos, stream_.end(),
                           [](char c) { return c == '<' || c == '>'; });
  ...
  {
    const size_t crSum =
      std::count(stream_.begin(), pos, '\r'); // carriage returns
...
private:
  ...
  const TokenList tokens{
    { "<?xml", Token::TK_DECL_BEGIN }, { "?>", Token::TK_DECL_END },
    { "</", Token::TK_LESS_SLASH },    { "/>", Token::TK_SLASH_GREATER },
    { "<", Token::TK_LESS }, // evaluate after TK_DECL_BEGIN!
    { ">", Token::TK_GREATER },        { "=", Token::TK_EQUAL },
    { "\"", Token::TK_QUOTE },         { "\'", Token::TK_QUOTE },
  };
...
private:
  ...
  void parseChildElements(XmlElement& parent)
  {
    while (token().type == Token::TK_LESS) {
      ...
      if (token().type != Token::TK_NAME || elementName != token().name)
...
inline XmlDoc parse(const std::string& stream) // throw XmlParsingError
{
